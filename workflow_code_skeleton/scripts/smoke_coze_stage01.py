...
def main() -> int:
    ...
    print(f"coze_api_token = {_status('COZE_API_TOKEN')}")
    print(f"coze_api_base = {os.getenv('COZE_API_BASE') or ''}")
    print(f"stage_key = {stage_key}")
    print(f"workflow_id_exists = {bool(workflow_id)}")
    ...
    try:
        result = coze_workflow_client.run_stage(
            stage_key,
            {
                "mode": "创作",
                "source_text": "测试短剧设定",
                "source_title": "Coze smoke test",
                "target_format": "短剧",
                "season_count": 1,
                "episodes_per_season": 1,
                "minutes_per_episode": 2,
                "adaptation_direction": "只做连通性测试。",
                "user_constraints": "",
                "user_requirements": "执行 stage 01 smoke test。",
            },
        )
