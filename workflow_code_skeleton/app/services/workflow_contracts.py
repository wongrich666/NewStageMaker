...
LEGACY_TOTAL_EPISODES = "total_episodes"
LEGACY_EPISODE_WORD_COUNT = "episode_word_count"
...
STAGE_FRAMEWORK_CAUSAL_CONFLICT_REWRITE = "framework_causal_conflict_rewrite"
STAGE_FRAMEWORK_CAUSAL_CONFLICT_MEMORY = "framework_causal_conflict_memory"
STAGE_FRAMEWORK_SCRIPT_WRITE = "framework_script_write"
STAGE_FRAMEWORK_SCRIPT_REVIEW = "framework_script_review"
STAGE_FRAMEWORK_SCRIPT_REWRITE = "framework_script_rewrite"
STAGE_FRAMEWORK_SCRIPT_MEMORY = "framework_script_memory"
STAGE_HOOKS = "hooks"
...
BATCH_ENRICHED_EPISODE_PLAN = "batchEnrichedEpisodePlan"
CONFLICT_START_EPISODE = "conflictStartEpisode"
BATCH_CAUSAL_CONFLICT_PLAN = "batchCausalConflictPlan"
BATCH_CAUSAL_CONFLICT_REVIEW = "batchCausalConflictReview"
CONFLICT_MEMORY = "conflictMemory"
SCRIPT_START_EPISODE = "scriptStartEpisode"
BATCH_SCRIPT_TEXT = "batchScriptText"
BATCH_SCRIPT_REVIEW = "batchScriptReview"
...
class WorkflowStageContract:
    ...
    def build_input_payload(self, variables: dict[str, Any]) -> dict[str, Any]:
        ...
        for name in self.input_names:
            if name in variables:
                ...
                payload[name] = ""
            elif name in {LAST_SUMMARY, HOOK_MEMORY, DIALOGUE_MEMORY, SCRIPT_MEMORY, CONFLICT_MEMORY}:
        ...
        if missing:
            joined = ", ".join(missing)
            raise ValueError(f"工作流 阶段 {self.stage_name} 缺少输入变量：{joined}")
    ...
    def validate_output_payload(self, output: Any) -> dict[str, Any]:
        if not isinstance(output, dict):
            ...
            raise ValueError(
                f"工作流 阶段 {self.stage_name} 输出必须是 object，实际得到 {actual_type}"
            )
        ...
        for name, type_name in self.output_types.items():
            ...
            try:
                if self.stage_name == STAGE_CONSISTENCY and name == IS_CONSISTENT:
                    ...
                    normalized[name] = coerce_strict_workflow_boolean(output[name])
                elif (
                    self.stage_name
                    in {
                        STAGE_HOOKS_REVIEW,
                    ...
                    normalized[name] = coerce_workflow_value(output[name], type_name)
            except ValueError as exc:
                value = output.get(name)
                if type_name == "string" and _is_empty_string_output(value):
                    raise ValueError(
                        f"工作流 阶段 {self.stage_name} 输出字段 {name} 不能为空"
                    ) from exc
                raise ValueError(
                    f"工作流 阶段 {self.stage_name} 输出字段 {name} 校验失败：{exc}"
                ) from exc
            # 这里只做“字段类型已对”还不够。
            ...
            if issue:
                raise ValueError(
                    f"工作流 阶段 {self.stage_name} 输出字段 {name} 结构不符合契约：{issue}"
                )
        if missing:
            joined = ", ".join(missing)
            if self.stage_name == STAGE_APPEARANCE_PRE_STRATEGY:
                raise ValueError(f"{self.stage_name} 缺少字段：{joined}")
            raise ValueError(f"工作流 阶段 {self.stage_name} 缺少输出变量：{joined}")
        return normalized
...
GLOBAL_VARIABLES: dict[str, WorkflowVariable] = {
    TOTAL_EPISODES: WorkflowVariable(
        TOTAL_EPISODES,
        "number",
        "总集数",
        "用户输入",
    ),
    EPISODE_PLAN: WorkflowVariable(
        EPISODE_PLAN,
        "string",
        "用户分集计划。framework 阶段可能先返回结构化 JSON；本地会把它序列化后缓存。consistency/worldview 阶段传全量计划；hooks/dialogues/script 阶段传当前批次规范化 JSON 字符串。",
        "用户输入/本地批次裁剪",
    ),
    USER_EXPECTATION: WorkflowVariable(
        USER_EXPECTATION,
        "string",
        "用户对剧本的期待/想要的故事。",
        "用户输入",
    ),
    CHARACTER_COUNT: WorkflowVariable(
        CHARACTER_COUNT,
        "number",
        "角色数量。",
        "用户输入",
    ),
    CHARACTER_APPEARANCE_REQUIREMENTS: WorkflowVariable(
        CHARACTER_APPEARANCE_REQUIREMENTS,
        "string",
        "供后续阶段复用的服装版本需求。默认由“服装前置策略生成器”自动生成，也兼容外部直接传入。",
        "工作流 输出/兼容外部输入",
    ),
    CHARACTER_ALIAS_NAMING_RULES: WorkflowVariable(
        CHARACTER_ALIAS_NAMING_RULES,
        "string",
        "供后续阶段复用的人物别名/服装版本命名偏好，例如“顾沉（上班）/顾沉（居家）”。默认由“服装前置策略生成器”自动生成。",
        "工作流 输出/兼容外部输入",
    ),
    OUTFIT_SWITCH_RULES: WorkflowVariable(
        OUTFIT_SWITCH_RULES,
        "string",
        "供后续阶段复用的服装切换规则。默认由“服装前置策略生成器”自动生成。",
        "工作流 输出/兼容外部输入",
    ),
    NORMALIZED_EPISODE_PLAN: WorkflowVariable(
        NORMALIZED_EPISODE_PLAN,
        "object",
        "代码侧保存的规范化分集计划对象。",
        "工作流 输出/本地缓存",
    ),
    STORY_OUTLINE: WorkflowVariable(
        STORY_OUTLINE,
        "string",
        "故事大纲。framework 阶段可先返回结构化 JSON，本地缓存时仍统一保存为字符串。",
        "框架阶段输出/用户输入兼容",
    ),
    USER_SCENES: WorkflowVariable(
        USER_SCENES,
        "string",
        "核心场景。framework 阶段可先返回结构化 JSON，本地缓存时仍统一保存为字符串。",
        "框架阶段输出/用户输入兼容",
    ),
    USER_CHARACTERS: WorkflowVariable(
        USER_CHARACTERS,
        "string",
        "人物小传。framework 阶段可先返回结构化 JSON，本地缓存时仍统一保存为字符串。",
        "框架阶段输出/用户输入兼容",
    ),
    CHARACTER_SEARCH_INTENT: WorkflowVariable(
        CHARACTER_SEARCH_INTENT,
        "string",
        "人设检索意图。当前人设工作流的新公开输入变量；未提供时代码侧兼容为空字符串，避免旧项目和旧前端缺字段时直接失败。",
        "用户输入/兼容缺省",
    ),
    script_title_content: WorkflowVariable(
        script_title_content,
        "string",
        "剧本标题。优先使用框架阶段生成标题；若缺失，再回退到本地基于用户想要的剧本生成的标题。",
        "框架阶段输出/本地回退",
    ),
    WORLDVIEW: WorkflowVariable(
        WORLDVIEW,
        "string",
        "生成的世界观内容",
        "工作流 输出",
    ),
    CHARACTERS: WorkflowVariable(
        CHARACTERS,
        "string",
        "生成的人设内容",
        "工作流 输出",
    ),
    SCENES: WorkflowVariable(
        SCENES,
        "string",
        "生成的核心场景内容",
        "工作流 输出",
    ),
    SCENE_APPEARANCE_REQUIREMENTS: WorkflowVariable(
        SCENE_APPEARANCE_REQUIREMENTS,
        "object",
        "从场景 JSON 中提炼出的视觉条件、造型条件、命名条件与 alias 使用规则摘要。",
        "本地从场景阶段结果提炼",
    ),
    APPEARANCE_MAPPING: WorkflowVariable(
        APPEARANCE_MAPPING,
        "object",
        "人物服装版本映射 JSON。内部保存 canonical 角色与 outfit variant / alias 的对应关系。",
        "工作流 输出/本地缓存",
    ),
    CHARACTER_REGISTRY: WorkflowVariable(
        CHARACTER_REGISTRY,
        "object",
        "角色基础身份注册表。保存 canonical 角色本体与稳定识别锚点。",
        "本地从 appearanceMapping 提炼",
    ),
    CHARACTER_ALIAS_REGISTRY: WorkflowVariable(
        CHARACTER_ALIAS_REGISTRY,
        "object",
        "角色别名注册表。保存 alias_name 与 canonical 角色的映射关系。",
        "本地从 appearanceMapping 提炼",
    ),
    EPISODE_ALIAS_PLAN: WorkflowVariable(
        EPISODE_ALIAS_PLAN,
        "object",
        "逐集 alias 使用计划。为当前批次切片后供 hooks / dialogues / script 使用。",
        "本地从 normalized_episode_plan + appearanceMapping 提炼",
    ),
    APPEARANCE_CONTINUITY_MEMORY: WorkflowVariable(
        APPEARANCE_CONTINUITY_MEMORY,
        "object",
        "跨批次保存的角色当前服装/别名状态记忆。",
        "本地维护",
    ),
    BATCH_HOOKS: WorkflowVariable(
        BATCH_HOOKS,
        "object",
        "当前批次 5 集的开头冲突钩子 JSON",
        "工作流 输出",
    ),
    ALL_HOOKS: WorkflowVariable(
        ALL_HOOKS,
        "object",
        "完整开头冲突钩子 JSON",
        "本地拼接",
    ),
    BATCH_DIALOGUES: WorkflowVariable(
        BATCH_DIALOGUES,
        "object",
        "当前批次 5 集的角色对话 JSON",
        "工作流 输出",
    ),
    ALL_DIALOGUES: WorkflowVariable(
        ALL_DIALOGUES,
        "object",
        "完整角色对话 JSON",
        "本地拼接",
    ),
    BATCH_SCRIPT: WorkflowVariable(
        BATCH_SCRIPT,
        "string",
        "当前批次 5 集的剧本正文",
        "工作流 输出",
    ),
    ALL_SCRIPT: WorkflowVariable(
        ALL_SCRIPT,
        "string",
        "完整剧本正文",
        "本地拼接",
    ),
    LAST_SUMMARY: WorkflowVariable(
        LAST_SUMMARY,
        "string",
        "最近一次剧本摘要，覆盖式保存",
        "工作流 输出/本地覆盖",
    ),
    FINAL_SCRIPT: WorkflowVariable(
        FINAL_SCRIPT,
        "string",
        "最终完整剧本",
        "工作流 输出",
    ),
    IS_CONSISTENT: WorkflowVariable(
        IS_CONSISTENT,
        "boolean",
        "集数一致性检查结果",
        "工作流 输出",
    ),
    EPISODE_WORD_COUNT: WorkflowVariable(
        EPISODE_WORD_COUNT,
        "number",
        "每集正文字数。仅当前 legacy 工作流 剧本正文工作流需要。",
        "用户输入",
    ),
    USER_CONTENT_BASELINE: WorkflowVariable(
        USER_CONTENT_BASELINE,
        "string",
        "用户内容提取基准 JSON。仅当前 legacy 工作流 工作流需要。",
        "本地整理",
    ),
    MAX_RETRIES: WorkflowVariable(
        MAX_RETRIES,
        "number",
        "工作流 内部审核修订最大轮次。仅当前 legacy 工作流需要。",
        "本地配置",
    ),
    BATCH_START_EPISODE: WorkflowVariable(
        BATCH_START_EPISODE,
        "number",
...
LEGACY_INPUT_ALIASES: dict[str, dict[str, LegacyInputAlias]] = {
    ...
    STAGE_DIALOGUES_REWRITE: {
        ...
        SCENES: DIALOGUE_SCENE_INPUT_VAR,
        ALL_HOOKS: DIALOGUE_HOOK_ALIASES,
        EPISODE_PLAN: (DIALOGUE_EPISODE_PLAN_INPUT_VAR, EPISODE_PLAN_VAR, "pxtQY7p2"),
        STORY_OUTLINE: STORY_OUTLINE_VAR,
        USER_SCENES: CORE_SCENE_INPUT_VAR,
        USER_CHARACTERS: CHARACTER_BIOS_VAR,
        EPISODE_WORD_COUNT: LEGACY_EPISODE_WORD_COUNT_ALIASES,
        APPEARANCE_MAPPING: APPEARANCE_MAPPING_VAR,
        TOTAL_EPISODES: DIALOGUE_TOTAL_EPISODES_INPUT_VAR,
        BATCH_START_EPISODE: DIALOGUE_START_INPUT_VAR,
        BATCH_DIALOGUES: DIALOGUE_CURRENT_ALIASES,
...
def _apply_legacy_workflow_id_aliases() -> None:
    framework_to_script_stages = {
        STAGE_FRAMEWORK_SCENE_DICTIONARY,
        STAGE_FRAMEWORK_APPEARANCE_MAPPING,
        STAGE_FRAMEWORK_ENRICHED_EPISODE_PLAN,
        STAGE_FRAMEWORK_CAUSAL_CONFLICT_WRITE,
    ...
    for stage_name, aliases in LEGACY_INPUT_ALIASES.items():
        if stage_name in framework_to_script_stages:
            continue
        for canonical_name, wire_name in list(aliases.items()):
            if wire_name == TOTAL_EPISODES_VAR:
                aliases[canonical_name] = LEGACY_TOTAL_EPISODES_ALIASES
            elif wire_name == EPISODE_WORD_COUNT_VAR:
                aliases[canonical_name] = LEGACY_EPISODE_WORD_COUNT_ALIASES
            elif wire_name == APPEARANCE_MAPPING_VAR:
                aliases[canonical_name] = LEGACY_APPEARANCE_MAPPING_ALIASES


_apply_legacy_workflow_id_aliases()
...
STAGE_CONTRACTS: dict[str, WorkflowStageContract] = {
    STAGE_FRAMEWORK: WorkflowStageContract(
        ...
        workflow_responsibility="根据用户想要的剧本、角色数量和总集数，生成剧本标题、故事大纲、人物小传、核心场景、分集计划。",
        local_responsibility="缓存并复用五项框架产物，后续阶段统一读取这些结果。",
        workflow_json_name="剧本框架撰写.json",
    ),
    STAGE_FRAMEWORK_NATURALIZE: WorkflowStageContract(
        ...
        workflow_responsibility="把完整结构化剧本框架改写成普通人可读的自然语言说明。",
    ...
    STAGE_APPEARANCE_PRE_STRATEGY: WorkflowStageContract(
        ...
        workflow_responsibility="基于故事、人物、场景和分集计划，先生成后续阶段要统一复用的服装版本需求、命名偏好和服装切换规则。",
        local_responsibility="缓存三项服装前置策略结果，并继续沿用现有逻辑字段供后续阶段读取。",
        workflow_json_name="服装前置策略生成器.json",
    ),
    STAGE_CONSISTENCY: WorkflowStageContract(
        ...
        workflow_responsibility="判断分集计划与总集数是否一致。",
        local_responsibility="不做内容判断，只根据布尔结果继续或停止。",
        workflow_json_name="集数一致性检查.json",
    ),
    STAGE_EPISODE_PLAN_NORMALIZE: WorkflowStageContract(
    ...
    STAGE_WORLDVIEW: WorkflowStageContract(
        ...
        workflow_responsibility="完成世界观提取、生成、审核、修订，返回最终可用世界观。",
        local_responsibility="不做业务审核循环，只校验 worldview 是否按契约返回并缓存。",
        workflow_json_name="02_世界观方案生成更新.json",
    ),
    STAGE_WORLDVIEW_NATURALIZE: WorkflowStageContract(
        ...
        workflow_responsibility="把结构化世界观改写成普通人可读的自然语言说明。",
        local_responsibility="保留结构化 worldview 正式产物，并额外缓存自然语言世界观说明供前端展示和辅助输入。",
        expected_output_kind="unstructured_natural_language_text",
        workflow_json_name="自然语言化.json",
    ),
    STAGE_CHARACTERS_NATURALIZE: WorkflowStageContract(
        ...
        workflow_responsibility="把结构化人物设定或原始人物小传整理成普通人可读的人物小传自然语言说明。",
        local_responsibility="保留结构化人物设定正式产物，并额外缓存统一自然语言人物小传供前端展示和导出使用。",
        expected_output_kind="unstructured_natural_language_text",
        workflow_json_name="自然语言化.json",
    ),
    STAGE_CHARACTERS: WorkflowStageContract(
    ...
    STAGE_SCENES: WorkflowStageContract(
        ...
        workflow_responsibility="完成核心场景提炼/复用、生成、审核、修订、整理。",
        local_responsibility="不做业务审核循环，只校验 scenes 是否按契约返回并缓存。",
        workflow_json_name="场景生成.json",
    ),
    STAGE_APPEARANCE_ALIAS_GENERATION: WorkflowStageContract(
        ...
        workflow_responsibility="逻辑阶段名；实际由 Python 依次调用服装映射编写、审核、修订与自然语言说明四个独立 workflow。",
        local_responsibility="编排四阶段循环，审核通过后才提交正式 appearanceMapping，并在本地提炼 registry / alias plan。",
    ),
    STAGE_APPEARANCE_ALIAS_WRITING: WorkflowStageContract(
        ...
        workflow_responsibility="编写完整结构化服装版本映射 JSON。",
    ...
    STAGE_APPEARANCE_ALIAS_REVIEW: WorkflowStageContract(
    ...
    STAGE_APPEARANCE_ALIAS_REWRITE: WorkflowStageContract(
        ...
        workflow_responsibility="基于当前服装映射与审核结果，输出新的完整 appearanceMapping。",
        local_responsibility="只做输出 repair/结构校验；是否继续循环由 Python 决定。",
        expected_output_kind="appearanceMapping_json",
        workflow_json_name="服装版本映射修订.json",
    ),
    STAGE_APPEARANCE_ALIAS_UNSTRUCTURED: WorkflowStageContract(
        ...
        workflow_responsibility="把正式结构化服装映射整理成便于人工阅读的自然语言说明。",
        local_responsibility="只缓存 c7VnQ4eX 展示文本；不能覆盖 h2KpLm91。",
        expected_output_kind="unstructured_natural_language_text",
        workflow_json_name="自然语言服装版本映射.json",
    ),
    STAGE_FRAMEWORK_SCENE_DICTIONARY: WorkflowStageContract(
        ...
        workflow_responsibility="把三幕十五节拍 stage07 framework_plan_package 资产化为 sceneDictionary 与正文世界规则摘要。",
        local_responsibility="作为三幕十五节拍框架转剧本专用链路的 08 阶段，不写入旧 scenes/all_hooks/all_dialogues。",
    ),
    STAGE_FRAMEWORK_APPEARANCE_MAPPING: WorkflowStageContract(
        stage_name=STAGE_FRAMEWORK_APPEARANCE_MAPPING,
        label="框架转剧本外观映射",
        input_names=(FRAMEWORK_PLAN_PACKAGE, CHARACTER_PLAN, SCENE_DICTIONARY, BEAT_CHECKPOINT_TIMELINE),
        output_types={APPEARANCE_MAPPING: "object"},
        output_aliases={APPEARANCE_MAPPING: (APPEARANCE_MAPPING_VAR,)},
        workflow_responsibility="基于最终策划包、人物方案与 sceneDictionary 生成 appearanceMapping。",
        local_responsibility="作为三幕十五节拍框架转剧本专用链路的 09 阶段，不复用旧 appearance_alias_generation。",
    ),
    STAGE_FRAMEWORK_ENRICHED_EPISODE_PLAN: WorkflowStageContract(
        ...
        workflow_responsibility="把 stage07 策划包扩展为可分批写作的 enrichedEpisodePlan。",
        local_responsibility="作为三幕十五节拍框架转剧本专用链路的 10 阶段，不写入旧 episode_plan/all_hooks。",
    ),
    STAGE_FRAMEWORK_CAUSAL_CONFLICT_WRITE: WorkflowStageContract(
        stage_name=STAGE_FRAMEWORK_CAUSAL_CONFLICT_WRITE,
        label="框架转剧本因果冲突推进计划编写",
        input_names=(
            TOTAL_EPISODES,
    ...
    STAGE_FRAMEWORK_CAUSAL_CONFLICT_REVIEW: WorkflowStageContract(
        ...
        output_aliases={
            REVIEW_PASSED: (BATCH_CAUSAL_CONFLICT_REVIEW_VAR, "approved"),
            REWRITE_REQUIRED: (BATCH_CAUSAL_CONFLICT_REVIEW_VAR,),
            BLOCKING_ISSUES: (BATCH_CAUSAL_CONFLICT_REVIEW_VAR,),
        },
        workflow_responsibility="审核当前批次因果冲突推进计划。",
        local_responsibility="只解析 review JSON 并驱动新链路修订循环，不接入旧 dialogues。",
        expected_output_kind="review_json",
    ),
    STAGE_FRAMEWORK_CAUSAL_CONFLICT_REWRITE: WorkflowStageContract(
        stage_name=STAGE_FRAMEWORK_CAUSAL_CONFLICT_REWRITE,
        label="框架转剧本因果冲突推进计划修订",
        input_names=(
            TOTAL_EPISODES,
            CONFLICT_START_EPISODE,
            BATCH_ENRICHED_EPISODE_PLAN,
        ...
        output_types={BATCH_CAUSAL_CONFLICT_PLAN: "object"},
        output_aliases={BATCH_CAUSAL_CONFLICT_PLAN: (BATCH_CAUSAL_CONFLICT_PLAN_VAR,)},
        workflow_responsibility="根据审核结果修订当前批次因果冲突推进计划。",
        local_responsibility="保留独立 batchCausalConflictPlan，不写入 batch_dialogues/all_dialogues。",
    ),
    STAGE_FRAMEWORK_CAUSAL_CONFLICT_MEMORY: WorkflowStageContract(
        stage_name=STAGE_FRAMEWORK_CAUSAL_CONFLICT_MEMORY,
        label="框架转剧本因果冲突记忆",
        input_names=(BATCH_CAUSAL_CONFLICT_PLAN, CONFLICT_MEMORY, CONFLICT_START_EPISODE),
        output_types={CONFLICT_MEMORY: "string"},
        output_aliases={CONFLICT_MEMORY: (CONFLICT_MEMORY_VAR,)},
        workflow_responsibility="基于当前批次因果冲突推进计划生成下一批 conflictMemory。",
        local_responsibility="维护新链路 conflictMemory，不复用旧 dialogue_memory。",
    ),
    STAGE_FRAMEWORK_SCRIPT_WRITE: WorkflowStageContract(
        ...
        output_types={BATCH_SCRIPT_TEXT: "string"},
        output_aliases={BATCH_SCRIPT_TEXT: (BATCH_SCRIPT_TEXT_VAR,)},
        workflow_responsibility="直接生成当前批次正文对白融合稿 batchScriptText。",
        local_responsibility="新链路正文阶段不依赖 all_dialogues，不调用旧 script_writing。",
        expected_output_kind="script_text",
    ),
    STAGE_FRAMEWORK_SCRIPT_REVIEW: WorkflowStageContract(
        ...
        output_aliases={
            REVIEW_PASSED: (BATCH_SCRIPT_REVIEW_VAR, "approved"),
            REWRITE_REQUIRED: (BATCH_SCRIPT_REVIEW_VAR,),
            BLOCKING_ISSUES: (BATCH_SCRIPT_REVIEW_VAR,),
        },
        workflow_responsibility="审核当前批次正文对白融合稿。",
        local_responsibility="只驱动 framework_script_rewrite，不复用旧 dialogue/script review。",
        expected_output_kind="review_json",
    ),
    STAGE_FRAMEWORK_SCRIPT_REWRITE: WorkflowStageContract(
        ...
        output_types={BATCH_SCRIPT_TEXT: "string"},
        output_aliases={BATCH_SCRIPT_TEXT: (BATCH_SCRIPT_TEXT_VAR,)},
        workflow_responsibility="根据审核结果修订当前批次正文对白融合稿。",
        local_responsibility="保留 batchScriptText，不写入 batch_dialogues/all_dialogues。",
        expected_output_kind="script_text",
    ),
    STAGE_FRAMEWORK_SCRIPT_MEMORY: WorkflowStageContract(
        stage_name=STAGE_FRAMEWORK_SCRIPT_MEMORY,
        label="框架转剧本正文记忆",
        input_names=(BATCH_SCRIPT_TEXT, SCRIPT_MEMORY, SCRIPT_START_EPISODE),
        output_types={SCRIPT_MEMORY: "string"},
        output_aliases={SCRIPT_MEMORY: (SCRIPT_MEMORY_VAR,)},
        workflow_responsibility="基于当前批次正文生成下一批 scriptMemory。",
        local_responsibility="维护新链路 scriptMemory，不复用旧 last_summary/script_memory。",
    ),
    STAGE_HOOKS: WorkflowStageContract(
        ...
        workflow_responsibility="生成当前批次 5 集的开头冲突钩子 JSON。",
        local_responsibility="划分批次、裁剪 episode_plan、拼接 all_hooks、推进批次。",
    ),
    STAGE_HOOKS_WRITING: WorkflowStageContract(
        ...
        workflow_responsibility="编写当前批次 5 集的开头冲突钩子 JSON。",
    ...
    STAGE_HOOKS_REVIEW: WorkflowStageContract(
    ...
    STAGE_DIALOGUES: WorkflowStageContract(
        ...
        workflow_responsibility="生成当前批次 5 集的角色对话 JSON。",
        local_responsibility="划分批次、裁剪 episode_plan、拼接 all_dialogues、推进批次。",
    ),
    STAGE_DIALOGUES_WRITING: WorkflowStageContract(
        ...
        workflow_responsibility="编写当前批次 5 集的角色对白 JSON。",
    ...
    STAGE_DIALOGUES_REVIEW: WorkflowStageContract(
        ...
        workflow_responsibility="审核当前批次角色对白是否可提交。",
        local_responsibility="把审核结果转成 passed/rewrite_required/blocking_issues，并决定是否进入修订。",
        expected_output_kind="review_json",
        workflow_json_name="角色对话审核.json",
    ),
    STAGE_DIALOGUES_REWRITE: WorkflowStageContract(
    ...
    STAGE_SCRIPT: WorkflowStageContract(
        ...
        workflow_responsibility="生成当前批次 5 集剧本正文。",
        local_responsibility="划分批次、裁剪 episode_plan、拼接 all_script、推进批次。",
    ),
    STAGE_SCRIPT_WRITING: WorkflowStageContract(
    ...
    STAGE_SCRIPT_REVIEW: WorkflowStageContract(
        ...
        workflow_responsibility="审核当前批次剧本正文是否可提交。",
        local_responsibility="把审核结果转成 passed/rewrite_required/blocking_issues，并决定是否进入修订。",
        expected_output_kind="review_json",
        workflow_json_name="剧本正文审核.json",
    ),
    STAGE_SCRIPT_REWRITE: WorkflowStageContract(
    ...
    STAGE_FINAL: WorkflowStageContract(
        ...
        workflow_responsibility="输出最终完整剧本。",
...
def _clone_stage_contract(
    ...
    return WorkflowStageContract(
        ...
        workflow_responsibility=source.workflow_responsibility,
...
STAGE_CONTRACTS[STAGE_HOOK_MEMORY] = WorkflowStageContract(
    ...
    workflow_responsibility="基于当前批次钩子和已有钩子记忆生成下一批可承接的钩子记忆。",
...
STAGE_CONTRACTS[STAGE_DIALOGUE_MEMORY] = WorkflowStageContract(
    ...
    workflow_responsibility="基于当前批次对白、钩子和已有对白记忆生成下一批可承接的对白记忆。",
...
def contract_for(stage_name: str) -> WorkflowStageContract:
    try:
        return STAGE_CONTRACTS[stage_name]
    except KeyError as exc:
        raise ValueError(f"未知 工作流 阶段：{stage_name}") from exc


def coerce_strict_workflow_boolean(value: Any) -> bool:
    ...
    if isinstance(value, dict):
        for key in ("is_consistent", "passed", "approved", "consistent"):
            if key in value:
                return coerce_strict_workflow_boolean(value[key])
    ...
    try:
        ...
        if parsed is not value:
            return coerce_strict_workflow_boolean(parsed)
    except Exception:
        pass
...
def coerce_workflow_value(value: Any, type_name: str) -> Any:
    if type_name == "boolean":
        ...
        if isinstance(value, dict):
            for key in ("is_consistent", "passed", "approved", "consistent"):
                if key in value:
                    return coerce_workflow_value(value[key], "boolean")

        text = str(value or "").strip()
        try:
            parsed = parse_json(text)
            if parsed is not value:
                return coerce_workflow_value(parsed, "boolean")
        except Exception:
            pass
    ...
    if type_name == "string":
        ...
        if not text:
            raise ValueError("工作流 输出 string 不能为空")
        return text
    ...
    raise ValueError(f"不支持的 工作流 类型：{type_name}")
