...
STAGE01_SOURCE_BRIEF_FIELDS = (
    ...
    "missing_information_risks",
)
STAGE01_DEPRECATED_WORKFLOW_INPUTS = frozenset(
    {
        "season_count",
        "episodes_per_season",
        "chars_per_episode",
...
def _writable_root() -> Path:
    """部署环境项目目录只读，通过 WRITABLE_ROOT 环境变量重定向写入路径。"""
...
def _payload_project_name(payload: Any) -> str:
    ...
    if not isinstance(payload, dict):
        return "未命名项目"
    for key in ("source_title", "project_title", "project_name", "title", "script_title", "name"):
    ...
    if isinstance(asset_state, dict):
        for key in ("source_title", "project_title", "title", "name"):
...
def _ensure_payload_project_title(payload: Any, project_id: Any = None) -> dict[str, Any]:
    ...
    project_name = source_title or project_name
    ...
    if isinstance(basic_config, dict):
        ...
        basic_title = str(basic.get("source_title") or "").strip() or project_name
        basic["source_title"] = basic_title
        basic["project_title"] = basic_title
        if not str(basic.get("title") or "").strip():
            basic["title"] = basic_title
...
def _history_project_dir(project_id: Any, project_name: Any = "") -> Path:
    ...
    path = _writable_root() / "cache" / safe_name
    path.mkdir(parents=True, exist_ok=True)
    return path


def _history_log_dir(project_id: Any, project_name: Any = "") -> Path:
    path = _writable_root() / "logs" / _project_history_id(project_id, project_name)
    path.mkdir(parents=True, exist_ok=True)
    return path
...
def _debug_project_dir(payload: Any) -> Path:
    ...
    path = _writable_root() / "debug" / _project_history_id(safe_payload.get("project_id"), project_name)
    path.mkdir(parents=True, exist_ok=True)
    return path
...
def _debug_cache_project_dir(payload: Any) -> Path:
    safe_payload = _ensure_payload_project_title(payload)
    project_name = _payload_project_name(safe_payload)
    path = _writable_root() / "cache" / _project_history_id(safe_payload.get("project_id"), project_name)
    path.mkdir(parents=True, exist_ok=True)
    return path
...
STAGE_DEFINITIONS: dict[str, FrameworkPlannerStageDefinition] = {
    "01": FrameworkPlannerStageDefinition(
        stage="01",
        label="原文信息提取",
        env_prefix="WORKFLOW_FRAMEWORK_01_SOURCE_BRIEF",
        ...
        input_aliases={
            ...
            "episodes_number": ("episodes_number", "total_episodes", "episodes_per_season"),
            "chars_per_epi": ("chars_per_epi", "chars_per_episode", "episode_word_count"),
    ...
    "02": FrameworkPlannerStageDefinition(
        stage="02",
        label="世界观方案生成更新",
        env_prefix="WORKFLOW_FRAMEWORK_02_WORLDVIEW",
    ...
    "03": FrameworkPlannerStageDefinition(
        stage="03",
        label="人设方案生成更新",
        env_prefix="WORKFLOW_FRAMEWORK_03_CHARACTER",
    ...
    "04": FrameworkPlannerStageDefinition(
        ...
        env_prefix="WORKFLOW_FRAMEWORK_04_BEAT",
    ...
    "05": FrameworkPlannerStageDefinition(
        stage="05",
        label="人物故事线生成更新",
        env_prefix="WORKFLOW_FRAMEWORK_05_STORYLINE",
    ...
    "06": FrameworkPlannerStageDefinition(
        stage="06",
        label="整体改编指引生成更新",
        env_prefix="WORKFLOW_FRAMEWORK_06_GUIDE",
    ...
    "07": FrameworkPlannerStageDefinition(
        stage="07",
        label="框架策划包校验",
        env_prefix="WORKFLOW_FRAMEWORK_07_PACKAGE",
...
def framework_planner_workflow_diagnostics(stage: str = "05") -> dict[str, Any]:
    definition = stage_definition(stage)
    diagnostics = _stage_runtime_diagnostics(definition, {})
    endpoint = str(diagnostics.get("resolved_url") or DEFAULT_TENCENT_ADP_API_URL)
...
def stage_has_real_backend(stage: str) -> bool:
    from .tencent_workflow_registry import api_key_for

    _, api_key = api_key_for(stage_definition(stage).stage)
    return bool(api_key)


def _parse_tencent_framework_response(
    ...
    if isinstance(response_json, dict):
        ...
        if code not in (None, 0, "0"):
            ...
            raise FrameworkPlannerStageError(
                ...
                status_code=502,
                detail={
                    "reason": str(message or "腾讯工作流 workflow returned failure"),
                    "tencent_code": code,
    ...
    if definition.stage == "01":
        extracted = _stage01_extract_payload_from_nested_response(response_json)
        if isinstance(extracted, dict) and isinstance(extracted.get("source_brief"), dict):
            return extracted

    # Stage04 必须先走专用解析。
    # 原因：腾讯工作流 当前返回是 data:String -> beat:String -> 真正业务 JSON。
    # 通用 parse_workflow_output 很容易只解析到 data.beat 的局部对象，然后丢失 15 节拍。
    if definition.stage == "04":
        extracted = _stage04_extract_payload_from_nested_tencent(response_json)

        if isinstance(extracted, dict) and isinstance(extracted.get("beat_checkpoint_timeline"), list):
            logger.warning(
                "[stage04 tencent parser] extracted nested payload directly: timeline_len=%s keys=%s",
        ...
        logger.warning(
            "[stage04 tencent parser] nested payload not found, fallback to generic parser; response_preview=%s",
...
def _run_framework_planner_stage_via_tencent(
    definition: FrameworkPlannerStageDefinition,
    payload: dict[str, Any] | None,
) -> dict[str, Any]:
    ...
    request_variables = _build_stage_request_variables(
        definition,
        normalized_payload,
        workflow_spec,
    )
    try:
        ...
        response_json = tencent_workflow_client.run_raw(
            definition.stage,
            request_variables,
        )
        if (
            definition.stage == "04"
            and _tencent_stage04_returned_single_beat_object(response_json)
        ):
            total_episodes = _total_episodes_from_payload(normalized_payload)
            retry_instruction = (
                "【上一轮输出纠错，最高优先级】上一轮错误地只返回了一个 beat 对象。"
                "本轮必须从头生成完整 JSON 对象，最外层必须包含 "
                "beat_checkpoint_timeline、checkpoint_explanation、display_text。"
                "beat_checkpoint_timeline 必须且只能包含 15 个节拍对象，beat_no 从 1 到 15；"
                "禁止直接返回单个节拍对象，禁止省略其余 14 条。"
                "每条必须填写 narrative_function、plot_content、character_change、"
                "conflict_upgrade、hook_or_reversal，且不得为空。"
                f"当前总集数为 {total_episodes}，episode_range 必须连续覆盖第1集到第{total_episodes}集。"
            )
            existing_feedback = str(request_variables.get("user_feedback") or "").strip()
            existing_score_report = str(
                request_variables.get("framework_score_report")
                or request_variables.get("framework_score_repo")
                or ""
            ).strip()
            retry_variables = dict(request_variables)
            retry_variables["user_feedback"] = "\n\n".join(
                item for item in (existing_feedback, retry_instruction) if item
            )
            retry_variables["framework_score_report"] = "\n\n".join(
                item for item in (existing_score_report, retry_instruction) if item
            )
            logger.warning(
                "stage04 腾讯工作流仅返回单个节拍，执行一次内容纠错重试：total_episodes=%s",
                total_episodes,
            )
            response_json = tencent_workflow_client.run_raw(
                definition.stage,
        ...
        debug_info = {}
        try:
            debug_info = tencent_workflow_client.get_last_stage_debug_info(definition.stage)
        except Exception:
            pass
        raise FrameworkPlannerStageError(
            f"阶段 {definition.stage} 腾讯工作流调用失败",
            stage=definition.stage,
            status_code=502,
            detail={
                "reason": str(exc),
    ...
    if (
        ...
        raise FrameworkPlannerStageError(
            "04 阶段腾讯工作流连续两次仅返回单个节拍，缺少完整十五节拍数组。",
            stage="04",
            status_code=422,
            detail={
                "reason": "tencent_stage04_returned_single_beat_object",
                "total_episodes": _total_episodes_from_payload(normalized_payload),
                "response_preview": safe_truncated_preview(response_json, limit=1200),
            },
        )

    parsed_response = _parse_tencent_framework_response(definition, response_json)
    try:
        ...
        raise FrameworkPlannerStageError(
            f"阶段 {definition.stage} 腾讯工作流输出解析失败",
            stage=definition.stage,
            status_code=502,
            detail={
                "reason": str(exc),
                "exception_type": type(exc).__name__,
                "response_preview": safe_truncated_preview(parsed_response, limit=1600),
            },
        ) from exc

    data, display_text, parse_warnings = _force_repair_stage_output_from_raw_workflow(
        definition=definition,
        response_json=parsed_response,
        data=data,
        display_text=display_text,
    ...
    return {
        ...
        "raw": {
            ...
            "workflow_json_path": str(workflow_spec.path),
            "workflow_contract_path": str(workflow_spec.contract_path) if workflow_spec.contract_path else "",
            "workflow_id": debug_info.get("workflow_id", ""),
            "response": parsed_response,
            "parse_warning": parse_warnings,
            "diagnostics": {
                **diagnostics,
                "api_key_source": debug_info.get("api_key_env", ""),
                "url_source": debug_info.get("api_url_source", ""),
                "resolved_url": debug_info.get("api_url", ""),
            },
        },
        "display_text": display_text,
...
def run_framework_planner_stage(stage: str, payload: dict[str, Any] | None) -> dict[str, Any]:
    definition = stage_definition(stage)
    return _run_framework_planner_stage_via_tencent(definition, payload)
...
def _normalize_payload(payload: dict[str, Any] | None) -> dict[str, Any]:
    ...
    basic_config = normalized.get("basic_config") if isinstance(normalized.get("basic_config"), dict) else {}
    if _is_blank(normalized.get("episodes_number")):
        episode_aliases = ("episodes_number", "total_episodes", "episodes_per_season")
        normalized["episodes_number"] = (
            _first_present_value(normalized, episode_aliases)
            or _first_present_value(basic_config, episode_aliases)
        )
    if _is_blank(normalized.get("chars_per_epi")):
        chars_aliases = ("chars_per_epi", "chars_per_episode", "episode_word_count")
        normalized["chars_per_epi"] = (
            _first_present_value(normalized, chars_aliases)
            or _first_present_value(basic_config, chars_aliases)
        )
...
def _build_stage_request_variables(
    ...
    for field in definition.input_fields:
        ...
        if cleaned_value is not value:
            logger.warning(
                "framework planner payload field cleaned before 工作流 request: stage=%s field=%s original_type=%s cleaned_type=%s original_preview=%s",
                definition.stage,
                field,
                type(value).__name__,
        ...
        for alias in aliases:
            if definition.stage == "01" and alias in STAGE01_DEPRECATED_WORKFLOW_INPUTS:
                continue
    ...
    for key, value in payload.items():
        if key in variables or _is_blank(value):
            continue
        if definition.stage == "01" and key in STAGE01_DEPRECATED_WORKFLOW_INPUTS:
            continue
    ...
    if _is_tencent_backend():
        _ensure_tencent_public_parameters(variables, workflow_spec, definition=definition)
...
def _ensure_tencent_public_parameters(
    variables: dict[str, Any],
    workflow_spec: FrameworkPlannerWorkflowSpec,
    *,
    definition: FrameworkPlannerStageDefinition | None = None,
) -> None:
    """腾讯工作流 validates declared start-node parameters by key presence."""
    public_keys = set(workflow_spec.public_variable_keys)
    if definition is not None and definition.stage == "01":
        public_keys.difference_update(STAGE01_DEPRECATED_WORKFLOW_INPUTS)
        public_keys.update(definition.input_fields)
...
def _json_objects_from_text(text: Any) -> list[Any]:
    ...
    工作流 某些 answerText/text.content 会出现：
    1. 单个 JSON 字符串；
    2. 两个 JSON 对象连续拼接；
    3. 前后带空白或非 JSON 文本。
...
def _iter_choice_message_texts(response_json: Any) -> list[str]:
    """兼容 工作流/OpenAI 风格 choices[].message.content。

    当前 工作流 返回里 content 可能不是 str，而是 list：
    [
      {"type": "reasoning", ...},
...
def _iter_workflow_structured_output_candidates(
    response_json: Any,
    output_fields: tuple[str, ...],
) -> list[tuple[str, Any]]:
    """从 工作流 原始返回中优先提取真正的阶段业务 JSON。
...
def _force_repair_stage_output_from_raw_workflow(
    *,
    definition: Any,
    response_json: Any,
    data: Any,
    ...
    candidates = _iter_workflow_structured_output_candidates(response_json, output_fields)
    if not candidates:
    ...
    for source, candidate in candidates:
        ...
        if copied_fields:
            warnings.append(
                f"已从 工作流 原始返回修复阶段输出 source={source} fields={','.join(copied_fields)}"
            )
            try:
                logger.warning(
...
def _extract_stage_output(
    *,
    ...
    if definition.stage == "01":
        extracted = _stage01_extract_payload_from_nested_response(response_json)
        if isinstance(extracted, dict) and isinstance(extracted.get("source_brief"), dict):
            parse_warnings.append("Stage01 已按 source_brief 新契约完成嵌套 JSON 提取")
    ...
    try:
        ...
        structured_candidates = _iter_workflow_structured_output_candidates(
            response_json,
...
def _stage04_repair_unquoted_json_string_values(text: str) -> str:
    """
    修复 Stage04 腾讯工作流/LLM 偶发的 JSON 漏引号问题。
...
def _stage04_try_parse_json_text(value: Any) -> Any:
    ...
    # 有些 腾讯工作流 字符串前后会夹杂少量说明，尝试截取最大 JSON 对象/数组。
    first_obj = text.find("{")
    last_obj = text.rfind("}")
    first_arr = text.find("[")
    last_arr = text.rfind("]")
...
def _stage01_looks_like_source_brief(value: Any) -> bool:
    ...
    return len(signal_fields.intersection(value.keys())) >= 3


def _stage01_extract_payload_from_nested_response(
    value: Any,
    *,
    depth: int = 0,
) -> dict[str, Any] | None:
    """从 腾讯工作流/工作流 的多层字符串包装中提取 Stage01 新版标准结构。"""
    if depth > 24:
        return None

    parsed = _stage04_deep_parse_json_like(value, depth=depth)
    if isinstance(parsed, dict):
        source_brief = parsed.get("source_brief")
        source_brief = _stage04_deep_parse_json_like(source_brief, depth=depth + 1)
        if isinstance(source_brief, dict):
            nested_brief = source_brief.get("source_brief")
            nested_brief = _stage04_deep_parse_json_like(nested_brief, depth=depth + 1)
            if isinstance(nested_brief, dict):
                source_brief = nested_brief
        ...
        if _stage01_looks_like_source_brief(parsed):
            return {"source_brief": parsed, "display_text": ""}
        ...
        for key in (
            ...
            if key not in parsed:
                continue
            found = _stage01_extract_payload_from_nested_response(
                parsed.get(key),
                depth=depth + 1,
            )
            if found:
                if not str(found.get("display_text") or "").strip():
                    display_text = parsed.get("display_text") or parsed.get("displayText") or ""
                    found["display_text"] = str(display_text or "").strip()
                return found

        for child in parsed.values():
            found = _stage01_extract_payload_from_nested_response(child, depth=depth + 1)
            if found:
                return found

    if isinstance(parsed, list):
        for item in parsed:
            found = _stage01_extract_payload_from_nested_response(item, depth=depth + 1)
            if found:
                return found
...
def _stage04_extract_payload_from_nested_tencent(value: Any, *, depth: int = 0) -> dict[str, Any] | None:
    """
    从任意 腾讯工作流 嵌套返回中提取 Stage04 标准结构。
    ...
    if isinstance(parsed, dict):
        ...
        for key in (
            ...
            if key in parsed:
                found = _stage04_extract_payload_from_nested_tencent(parsed.get(key), depth=depth + 1)
                if found:
                    return found

        # 兼容 腾讯工作流 outputs / variables / newVariables / responseData 等容器。
        for container_key in (
            ...
            found = _stage04_extract_payload_from_nested_tencent(container, depth=depth + 1)
            if found:
                return found

        # 最后兜底遍历所有字段。
        for child in parsed.values():
            found = _stage04_extract_payload_from_nested_tencent(child, depth=depth + 1)
            if found:
                return found

    if isinstance(parsed, list):
        for item in parsed:
            found = _stage04_extract_payload_from_nested_tencent(item, depth=depth + 1)
            if found:
                return found
...
def _find_display_text_recursive(value: Any, *, depth: int = 0) -> str:
    """递归查找 display_text / displayText，用于兼容 腾讯工作流 嵌套输出。"""
    if depth > 12:
        return ""
...
def _extract_display_text(
    ...
    *,
    ...
    recursive_display_text_stages = {"01", "06"}

    # 2. 对已确认有嵌套 display_text 的阶段开启递归查找。
    # Stage01: 腾讯工作流 常返回 data.confirmed_info.display_text。
    # Stage06: 腾讯工作流 常返回 data.adaptation.display_text。
    if str(stage).zfill(2) in recursive_display_text_stages:
    ...
    # 3. 保留原来的 normalize 逻辑，兼容旧 工作流 / OpenAI 风格。
    try:
        root_response = normalize_stage_response(
            response_json,
            stage=stage,
...
def normalize_stage_response(
    ...
    *,
    ...
    """Normalize 01~07 工作流 stage output into a dict so downstream .get() calls stay safe."""
    try:
...
def _compact_stage_05_basic_config(value: Any) -> dict[str, Any]:
    ...
    source_title = str(compact.get("source_title") or "").strip()
    project_title = str(compact.get("project_title") or "").strip()
    title = source_title or project_title
    if title:
        compact["source_title"] = title
        compact["project_title"] = title
    return compact
    ...
    for source in sources:
        ...
        for key in ("display_text", "displayText"):
            ...
            if isinstance(value, str) and value.strip():
                ...
                return result
        found = _find_display_text_recursive(source)
        if found:
            result["display_text"] = found
            return result
    return result
...
def _stage_runtime_diagnostics(
    ...
    from .tencent_workflow_registry import api_key_for, api_url_for, timeout_seconds, workflow_spec

    spec = workflow_spec(definition.stage)
    api_key_source, api_key = api_key_for(definition.stage)
    url_source, resolved_url = api_url_for(definition.stage)
    configured_url = resolved_url if url_source != "default" else ""
    workflow_id_source, workflow_id = "registry", spec.workflow_id
...
def _log_stage_entry(
    ...
    logger.info(
        ...
        diagnostics.get("resolved_url") or DEFAULT_TENCENT_ADP_API_URL,
...
def _log_stage_05_input_diagnostics(
    ...
    for field in (
        ...
        if suspected:
            logger.warning(
                "stage=05 输入疑似污染：field=%s 包含 工作流 raw response，请检查前端 state 保存逻辑或后端业务字段提取逻辑",
                field,
            )
...
def _pollution_keys_in_value(value: Any) -> list[str]:
    ...
    def visit(item: Any, depth: int = 0) -> None:
        if depth > 4 or len(found) == len(WORKFLOW_RAW_RESPONSE_KEYS):
            return
        if isinstance(item, str):
            for key in WORKFLOW_RAW_RESPONSE_KEYS:
                if key in item:
                    found.add(key)
        ...
        if isinstance(item, dict):
            for key, nested in item.items():
                key_text = str(key)
                if key_text in WORKFLOW_RAW_RESPONSE_KEYS:
                    found.add(key_text)
    ...
    visit(value)
    return [key for key in WORKFLOW_RAW_RESPONSE_KEYS if key in found]
    ...
    def text_value(*keys: str) -> str:
        for key in keys:
            ...
            if isinstance(candidate, list):
                return "、".join(_normalize_string_list(candidate))
            return str(candidate).strip()
        return ""

    episodes_number = text_value(
        "episodes_number",
        "total_episodes",
        "episodes_per_season",
        "episode_count",
    )
    episode_match = re.search(r"\d+", episodes_number)
    if episode_match:
        episodes_number = str(int(episode_match.group(0)))

    chars_text = text_value(
        "chars_per_epi",
        "chars_per_episode",
        "episode_word_count",
    )
    chars_match = re.search(r"\d+", chars_text)
    chars_per_epi = int(chars_match.group(0)) if chars_match else 600

    normalized = {
        "title": text_value("title", "source_title", "project_title") or "未命名项目",
        "source_type": text_value("source_type", "sourceType"),
        "genre": text_value("genre"),
        "tone": text_value("tone", "tone_keywords"),
        "target_format": text_value("target_format", "format") or "短剧",
        "episodes_number": episodes_number,
        "chars_per_epi": chars_per_epi,
        "adaptation_direction": text_value("adaptation_direction", "direction"),
        "core_logline": text_value("core_logline", "logline", "core_premise"),
        "protagonist": text_value("protagonist", "main_character", "mainCharacter"),
        "main_opposition": text_value(
            "main_opposition",
            "main_antagonist",
            "antagonist",
            "opposition",
        ),
        "core_conflict": text_value("core_conflict", "main_conflict", "conflict"),
        "must_keep_elements": _normalize_string_list(source.get("must_keep_elements")),
        "forbidden_deviations": _normalize_string_list(source.get("forbidden_deviations")),
        "available_material_summary": text_value(
            "available_material_summary",
            "story_outline",
            "outline",
            "summary",
        ...
        "missing_information_risks": _normalize_string_list(source.get("missing_information_risks")),
    }
    return {field: normalized[field] for field in STAGE01_SOURCE_BRIEF_FIELDS}
...
CHARACTER_SINGLE_ENTRY_DETAIL_KEYS = (
    "role",
    "role_type",
    "roleType",
    "character_role",
...
def _extract_single_character_entry(value: dict[str, Any]) -> dict[str, Any]:
    source = value if isinstance(value, dict) else {}
    item = {
        key: source[key]
        for key in CHARACTER_SINGLE_ENTRY_KEYS
        if key in source and source.get(key) not in (None, "", [], {})
    }
    if not item:
        return {}
    if _first_present_value(item, CHARACTER_NAME_ALIASES) is None:
        return {}
    if _first_present_value(item, CHARACTER_SINGLE_ENTRY_DETAIL_KEYS) is None:
        return {}
    return _normalize_character_entry(item, index=1)


def _is_placeholder_character(value: Any) -> bool:
    if not isinstance(value, dict):
        return False
    name = str(value.get("name") or "").strip()
    if not name:
        return False
    meaningful_keys = [key for key, item in value.items() if key != "name" and item not in (None, "", [], {})]
    return not meaningful_keys and (name.startswith("角色") or name.lower().startswith("character"))


def _ensure_character_core_fields(value: dict[str, Any]) -> dict[str, Any]:
    result = dict(value if isinstance(value, dict) else {})
    single_character = _extract_single_character_entry(result)
    ...
    if isinstance(characters, list) and characters:
        ...
        normalized_characters = []
    if single_character and (
        not normalized_characters
        or all(_is_placeholder_character(item) for item in normalized_characters)
    ):
        normalized_characters = [single_character]
        result["characters"] = normalized_characters
    ...
    if single_character and (not protagonist or _is_placeholder_character(protagonist)):
        protagonist = single_character
    ...
    if single_character and (
        not main_characters
        or all(_is_placeholder_character(item) for item in main_characters)
    ):
        main_characters = [single_character]
...
def _total_episodes_from_payload(payload: dict[str, Any] | None) -> int:
    ...
    for candidate in (
        source.get("episodes_number"),
        source.get("total_episodes"),
        basic.get("episodes_number"),
        basic.get("total_episodes"),
        source_brief.get("episodes_number"),
    ):
...
def _tencent_data_keys(value: Any) -> list[str]:
    """调试用：尽量展示 腾讯工作流 data 层的 key。"""
...
def _tencent_stage04_returned_single_beat_object(value: Any) -> bool:
    """
    判断 腾讯工作流 是否只返回了单个 beat 对象。
...
def _stage04_raw_timeline_value(value: Any) -> Any:
    ...
    仅用于校验 detail；不负责放行。
    """
    extracted = _stage04_extract_payload_from_nested_tencent(value)
...
def _validate_stage04_output_or_raise(
    *,
    ...
    detail: dict[str, Any] = {
        ...
        "tencent_data_keys": _tencent_data_keys(raw_response_json if raw_response_json is not None else response_json),
    ...
    if _tencent_stage04_returned_single_beat_object(raw_response_json if raw_response_json is not None else response_json):
        failures.append("tencent_stage04_returned_single_beat_object")
...
def _build_mock_stage_output(stage: str, payload: dict[str, Any]) -> tuple[dict[str, Any], str]:
    if stage == "01":
        ...
        source_brief = {
            ...
            "core_logline": source_text[:180] or "当前为 mock 提取结果，请在接入真实原文后替换。",
            "protagonist": "待从原文提取",
            "main_opposition": "旧秩序与持续升级的外部阻力",
            "core_conflict": "主角被旧秩序压制，被迫进入更高风险的规则体系中完成自我翻盘。",
            "must_keep_elements": [],
            "forbidden_deviations": [],
            "available_material_summary": source_text[:500],
            "missing_information_risks": [],
        }
        display_text = json.dumps(source_brief, ensure_ascii=False, indent=2)
        return {"source_brief": source_brief}, display_text
...
def _is_tencent_backend() -> bool:
    return True


def _normalize_workflow_url(raw_url: str) -> str:
    url = str(raw_url or "").strip().rstrip("/")
    if not url:
        raise ValueError("工作流 接口地址不能为空")
    if not url.startswith(("http://", "https://")):
        raise ValueError(f"工作流 接口地址必须以 http:// 或 https:// 开头：{url}")
...
def _overlay_source_brief_locked_fields(
    ...
    def config_value(*keys: str) -> Any:
        for key in keys:
            candidate = _payload_config_value(payload, key)
            if candidate not in (None, "", [], {}):
                return candidate
        return None

    locked_fields = {
        "title": config_value("source_title", "title", "project_title"),
        "target_format": config_value("target_format"),
        "episodes_number": config_value(
            "episodes_number",
            "total_episodes",
            "episodes_per_season",
        ),
        "chars_per_epi": config_value(
            "chars_per_epi",
            "chars_per_episode",
            "episode_word_count",
        ),
        "adaptation_direction": config_value("adaptation_direction"),
    }
    for key, candidate in locked_fields.items():
        if candidate not in (None, "", [], {}):
...
def _repair_stage_output_with_payload(
    ...
    if stage == "07":
        ...
        # 腾讯工作流 Stage07 可能只返回很薄的 framework/package，甚至被清洗成空壳。
        ...
        # 关键：用修复后的 package 重新生成 validation_report，覆盖 腾讯工作流/旧清洗得到的空壳校验。
