...
def create_app(*, workflow_spec_path: str | None = None) -> Flask:
    ...
    def run_framework_to_script_stage08_api():
        ...
        try:
            try:
                from .services.coze_workflow_client import coze_workflow_client

                raw_output = coze_workflow_client.run_stage(
                    "stage_08",
                ...
                return _json_error(
                    str(exc),
                    status=500,
                    fallback="08 场景字典提炼调用失败，请检查 COZE_API_TOKEN、COZE_WORKFLOW_STAGE_08_ID 和工作流变量。",
                )
        ...
        try:
            try:
                ...
                return _json_error(
                    str(exc),
                    status=500,
                    fallback="09 人设服装 alias 映射调用失败，请检查 COZE_API_TOKEN、COZE_WORKFLOW_STAGE_09_ID 和工作流变量。",
                )
        ...
        try:
            try:
                ...
                return _json_error(
                    str(exc),
                    status=500,
                    fallback="10 分集细化方案调用失败，请检查 COZE_API_TOKEN、COZE_WORKFLOW_STAGE_10_ID 和工作流变量。",
                )
    ...
    def run_framework_to_script_stage11_api():
        ...
        try:
            ...
            total_episodes = 0
            try:
                from .services.coze_workflow_client import CozeWorkflowError, coze_workflow_client
                ...
                logger.info(
                    ...
                    {
                        "write": "stage_11_write",
                        "review": "stage_11_review",
                        "rewrite": "stage_11_rewrite",
                        "memory": "stage_11_memory",
                    },
                )
                ...
                for retry_count in range(max_write_retries + 1):
                    ...
                    try:
                        write_output = coze_workflow_client.run_stage("stage_11_write", project_state=base_vars)
                ...
                for review_round in range(1, max_review_rounds + 1):
                    ...
                    try:
                        review_output = coze_workflow_client.run_stage(
                            "stage_11_review",
                            project_state={
                                **base_vars,
                    ...
                    rewrite_output = coze_workflow_client.run_stage(
                        "stage_11_rewrite",
                        project_state=rewrite_vars,
                    )
                    logger.info(
                ...
                try:
                    memory_output = coze_workflow_client.run_stage(
                        "stage_11_memory",
                        project_state={
                            "batchCausalConflictPlan": conflict_plan,
                            "conflictMemory": conflict_memory,
    ...
    def run_framework_to_script_stage12_api():
        ...
        try:
            ...
            try:
                ...
                conflict_episodes = conflict_plan.get("episodes") if isinstance(conflict_plan, dict) else []
                stage_names = {
                    "script_write": "stage_12_write",
                    "script_review": "stage_12_review",
                    "script_rewrite": "stage_12_rewrite",
                    "script_memory": "stage_12_memory",
                }
                logger.info(
                ...
                write_output = coze_workflow_client.run_stage("stage_12_write", project_state=base_vars)
