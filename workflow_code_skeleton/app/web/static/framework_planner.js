(function () {
  ...
  const ASSET_SYNC_SOURCE = `framework-planner-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const DEFAULT_SEASON_COUNT = 1;
  const DEFAULT_EPISODE_WORD_COUNT = 600;
  ...
  const SOURCE_BRIEF_GROUPS = [
    ["基础信息", ["title", "source_type", "genre", "tone", "target_format", "episodes_per_season", "episode_word_count"]],
  ...
  const BASIC_REQUIRED_FIELDS = [
    { key: "source_title", label: "作品标题", reason: "第一阶段必填", scopes: ["basic"] },
    { key: "mode", label: "写作模式", reason: "第一阶段必填", scopes: ["basic"] },
    { key: "target_format", label: "目标形式", reason: "第一阶段必填", scopes: ["basic"] },
    { key: "episodes_per_season", label: "总集数", reason: "第一阶段及 04 节拍规划必填", positiveInteger: true, scopes: ["basic", "beat", "script"] },
    { key: "episode_word_count", label: "每集字数", reason: "第一阶段及后续剧本生成必填", positiveInteger: true, scopes: ["basic", "script"] },
    { key: "adaptation_direction", label: "改编方向", reason: "第一阶段必填", scopes: ["basic"] },
    { key: "user_requirements", label: "用户提示词", reason: "第一阶段必填", scopes: ["basic"] },
    { key: "source_text", label: "原文材料", reason: "第一阶段必填", scopes: ["basic"] },
    { key: "user_constraints", label: "限制条件", reason: "第一阶段必填", scopes: ["basic"] },
  ];
  const BASIC_REQUIRED_FIELD_KEYS = new Set(BASIC_REQUIRED_FIELDS.map((item) => item.key));
  ...
  const initialState = {
    ...
    basic_config: {
      ...
      source_title: "",
      target_format: "短剧",
      season_count: DEFAULT_SEASON_COUNT,
      episodes_per_season: "",
      total_episodes: "",
      episode_word_count: DEFAULT_EPISODE_WORD_COUNT,
      chars_per_episode: DEFAULT_EPISODE_WORD_COUNT,
      adaptation_direction: "",
      user_constraints: "",
      user_requirements: "",
    },
    ...
    prompt_preferences: {
      ...
      manual_stage_prompts: normalizeStagePrompts({}),
      tag_stage_prompts: normalizeStagePrompts({}),
      basic_prompt_fields: {
        adaptation_direction: "",
        user_constraints: "",
        user_requirements: "",
  ...
  const realApi = {
    ...
    async startFrameworkScript(payload) {
      ...
      try {
        ...
        throw new Error(`框架转剧本接口返回非 JSON 响应，请多次尝试生成：status=${response.status} contentType=${response.headers.get("content-type") || ""} body=${snippet}`);
      }
    ...
    async saveFrameworkAsset(payload) {
      ...
      const data = await response.json().catch(() => ({
        success: false,
        message: "保存接口返回了无法解析的响应，请多次尝试生成",
      }));
      if (!response.ok || data.success === false || data.ok === false) {
  ...
  async function requestJson(url, options) {
    ...
    if (!["GET", "HEAD"].includes(method) && String(url || "").startsWith("/api/user-knowledge")) {
      announceAssetChange("knowledge-updated", { method });
    }
    return data;
  }
  ...
  function scheduleAssetRefresh() {
    ...
    assetRefreshTimer = window.setTimeout(() => {
      ...
      assetRefreshInFlight = Promise.all([
  ...
  function currentProjectCacheName() {
    ...
    const rawName = String(configState.source_title || configState.project_title || "未命名项目").trim() || "未命名项目";
    return safeProjectCacheName(rawName);
  }

  function syncBasicTitleFields(basicConfig, options) {
    const basic = basicConfig && typeof basicConfig === "object" ? basicConfig : {};
    const allowProjectFallback = !(options && options.allowProjectFallback === false);
    const sourceTitle = String(basic.source_title || "").trim();
    const fallbackTitle = allowProjectFallback ? String(basic.project_title || "").trim() : "";
    const title = sourceTitle || fallbackTitle;
    basic.source_title = title;
    basic.project_title = title;
    return title;
  }

  function syncBasicTitles(options) {
    if (!state || !state.basic_config) return "";
    return syncBasicTitleFields(state.basic_config, options);
  }
  ...
  function normalizeState(saved) {
    ...
    syncBasicTitleFields(next.basic_config);
    ...
    next.prompt_preferences = normalizePromptPreferences(Object.assign({}, storedPreferences || {}, savedPreferences || {}, {
      manual_stage_prompts: mergeStagePromptsNonEmpty(
        (storedPreferences || {}).manual_stage_prompts || (storedPreferences || {}).stage_prompts || {},
        (savedPreferences || {}).manual_stage_prompts || (savedPreferences || {}).stage_prompts || {}
      ),
      tag_stage_prompts: mergeStagePromptsNonEmpty(
        (storedPreferences || {}).tag_stage_prompts || {},
        (savedPreferences || {}).tag_stage_prompts || {}
      ),
    }));
  ...
  function normalizePromptPreferences(value) {
    ...
    const hasSplitPrompts = Object.prototype.hasOwnProperty.call(source, "manual_stage_prompts")
      || Object.prototype.hasOwnProperty.call(source, "tag_stage_prompts");
    next.manual_stage_prompts = normalizeStagePrompts(
      hasSplitPrompts ? source.manual_stage_prompts || {} : source.stage_prompts || {}
    );
    next.tag_stage_prompts = normalizeStagePrompts(source.tag_stage_prompts || {});
    next.stage_prompts = mergeStagePromptsNonEmpty(next.manual_stage_prompts, next.tag_stage_prompts);
  ...
  function savePromptPreferences(reason) {
    ...
    const title = state.basic_config.source_title || state.basic_config.project_title || "";
    ...
    preferenceSyncTimer = window.setTimeout(() => {
      requestJson("/api/user-knowledge/preferences", {
        ...
        body: JSON.stringify({
          ...
          manual_stage_prompts: normalizeStagePrompts((state.prompt_preferences || {}).manual_stage_prompts || {}),
          tag_stage_prompts: normalizeStagePrompts((state.prompt_preferences || {}).tag_stage_prompts || {}),
  ...
  function showToast(message, options = {}) {
    ...
    if (shouldRender) {
      ...
      const toastElement = app.querySelector("[data-fp-toast]");
      if (toastElement) {
        toastElement.textContent = ui.toast;
        toastElement.hidden = !ui.toast;
      } else {
    ...
    showToast.timer = window.setTimeout(() => {
      ...
      if (shouldRender) {
        ...
        const currentToastElement = app.querySelector("[data-fp-toast]");
        if (currentToastElement) {
          currentToastElement.textContent = "";
          currentToastElement.hidden = true;
        }
      }
    }, 2400);
  }
  ...
  function setCurrentView(viewId) {
    ...
    render();
    if (DEV_LOG_ENABLED) loadStageHistory(stageKeyForView(viewId)).catch(() => {});
  }
  ...
  function renderSaveFrameworkButton(sizeClass) {
    ...
    return `<button class="fp-btn fp-save-btn${className}" data-action="save-framework-asset" ${disabled}>${label}</button>`;
  }
  ...
  function updateRunningTimerDom() {
    const stageKey = runningStageKey();
    if (!stageKey) return;
    const elapsedText = processingElapsedLabel(stageKey);
    document.querySelectorAll("[data-running-elapsed]").forEach((node) => {
      node.textContent = elapsedText;
    });
    const statusNode = document.querySelector("[data-brand-motion-status]");
    if (statusNode) {
      statusNode.textContent = `正在处理 ${stageNoForKey(stageKey)} · ${stageDisplayTitle(stageKey)}`;
    }
  }
  ...
  function stageDependencyIssues(stageKey) {
    ...
    const hasBasic = !isEmptyValue(state.basic_config)
      && (String(state.basic_config.source_title || state.basic_config.project_title || "").trim()
  ...
  function stagePreferencePlaceholder(stageKey) {
    return {
      ...
      appearance: "填写该标签在 09 角色外观匹配场景阶段要额外注入的偏好，例如外观识别点、服装风格和别名一致性。",
  ...
  function stageRunBlockReason(stageKey) {
    ...
    if (stageKey === "beat") {
      const missing = missingBasicRequiredFields("beat");
      if (missing.length) return "缺少总集数，请回到基础配置填写后再生成 04 阶段。";
    }
    return "";
  }
  ...
  function knowledgePayloadFields(stageKey) {
    ...
    const manualStagePrompts = normalizeStagePrompts((state.prompt_preferences || {}).manual_stage_prompts || {});
    const tagStagePrompts = normalizeStagePrompts((state.prompt_preferences || {}).tag_stage_prompts || {});
    ...
    return {
      ...
      prompt_preferences: {
        ...
        manual_stage_prompts: manualStagePrompts,
        tag_stage_prompts: tagStagePrompts,
  ...
  function buildPreferenceSnapshot() {
    const tags = selectedKnowledgeTags();
    const selectedIds = (ui.knowledge.selectedIds || []).map((item) => String(item || "").trim()).filter(Boolean);
    const mergedStagePrompts = normalizeStagePrompts((state.prompt_preferences || {}).stage_prompts || {});
  ...
  function restoreScrollPosition(snapshot) {
    ...
    window.requestAnimationFrame(() => {
      ...
      const main = app.querySelector(".fp-main");
      if (main && Number.isFinite(snapshot.mainTop)) main.scrollTop = snapshot.mainTop;
      if (main && Number.isFinite(snapshot.mainLeft)) main.scrollLeft = snapshot.mainLeft;
    });
  }
  ...
  function render(options = {}) {
    ...
    if (!options.skipSync) syncStageFlow(state);
    if (!options.skipPersist) saveState();
    app.innerHTML = `
      <div class="fp-shell">
        ${renderSidebar()}
        <main class="fp-main">
        ...
        ${renderFooter()}
        <div class="fp-toast" data-fp-toast ${ui.toast ? "" : "hidden"}>${escapeHtml(ui.toast)}</div>
        ...
        ${ui.validationPrompt ? renderValidationPrompt() : ""}
        ${ui.assetImporting ? renderAssetImportOverlay() : ""}
      </div>
    `;
  ...
  function renderValidationPrompt() {
    const prompt = ui.validationPrompt || {};
    const fields = Array.isArray(prompt.fields) ? prompt.fields : [];
    return `
      <div class="fp-modal-mask" data-action="close-validation-prompt">
        <div class="fp-modal fp-validation-modal" data-modal-content="validation">
          <div class="fp-modal-head">
            <div>
              <h2>${escapeHtml(prompt.title || "请补全必填信息")}</h2>
              <p class="fp-modal-sub">${escapeHtml(prompt.message || "带星号的字段需要先填写，后续阶段会依赖这些基础信息。")}</p>
            </div>
          </div>
          <div class="fp-validation-list">
            ${fields.map((item) => `
              <div class="fp-validation-item">
                <strong>${escapeHtml(item.label || item.key || "未命名字段")}</strong>
                <span>${escapeHtml(item.reason || "必填")}</span>
              </div>
            `).join("") || `<div class="fp-validation-item"><strong>未知字段</strong><span>请检查基础配置。</span></div>`}
          </div>
          <div class="fp-actions">
            <button class="fp-btn primary" data-action="close-validation-prompt">知道了，去填写</button>
          </div>
        </div>
      </div>
    `;
    ...
    return `
      <div class="fp-modal-mask" data-action="close-new-script">
        <div class="fp-modal" data-modal-content="new-script">
          ...
          <label class="fp-field" style="margin-top:12px"><span>总集数</span><input type="number" min="1" data-new-script-field="episodes_per_season" value="${escapeHtml(form.episodes_per_season)}" /></label>
  ...
  function renderAssetItem(item) {
    ...
    const status = String(item.runtime_status || item.status || "in_progress");
    ...
    const scriptStatus = item.has_script_asset
      ? (item.script_locked ? "关联剧本：已锁定" : `关联剧本：${item.script_asset_status_label || "生成中"}`)
      : "关联剧本：未创建";
    return `
      <article class="fp-asset-item">
        <div>
          ...
          <div class="fp-asset-meta">
            <span class="fp-tag fp-asset-status-tag ${assetStatusClass(item)}">${escapeHtml(assetStatusLabel(item))}</span>
            <span>上次修改：${escapeHtml(formatDateTime(item.updated_at || item.created_at || ""))}</span>
            <span>${escapeHtml(scriptStatus)}</span>
            ${item.script_asset_updated_at ? `<span>剧本更新：${escapeHtml(formatDateTime(item.script_asset_updated_at))}</span>` : ""}
          </div>
  ...
  function renderKnowledgePanel() {
    const { selectedTags, missingIds, selectedLabel } = knowledgeSelectionData();
      ...
      <section class="fp-card fp-knowledge-panel ${ui.knowledge.open ? "is-open" : ""}">
        <button class="fp-knowledge-toggle" type="button" data-action="toggle-knowledge-panel" aria-expanded="${ui.knowledge.open ? "true" : "false"}">
          ...
          <strong>智慧库 / 阶段偏好</strong>
            <small>${escapeHtml(knowledgeSelectionSummaryText(selectedLabel))}</small>
          </span>
  ...
  function knowledgeSelectionData() {
    const selectedTags = selectedKnowledgeTags();
    const selectedTagIds = new Set(selectedTags.map((tag) => String(tag.id || "")));
    const missingIds = (ui.knowledge.selectedIds || [])
      .map((id) => String(id || "").trim())
      .filter((id) => id && !selectedTagIds.has(id));
    const selectedLabel = selectedTags.length || missingIds.length
      ? `${selectedTags.length + missingIds.length} 个已选`
      : "允许不选择";
    return { selectedTags, missingIds, selectedLabel };
  }

  function knowledgeSelectionSummaryText(selectedLabel) {
    return `${selectedLabel} · ${ui.knowledge.status ? ui.knowledge.status : "可为每个策划阶段注入不同偏好"}`;
  }

  function refreshKnowledgeSelectionUi() {
    const panel = app.querySelector(".fp-knowledge-panel");
    if (!panel) return false;
    const { selectedTags, missingIds, selectedLabel } = knowledgeSelectionData();
    const summary = panel.querySelector(".fp-knowledge-toggle small");
    if (summary) summary.textContent = knowledgeSelectionSummaryText(selectedLabel);
    const selectedBlock = panel.querySelector(".fp-knowledge-selected");
    if (selectedBlock) selectedBlock.outerHTML = renderKnowledgeSelected(selectedTags, missingIds);
    const selectedIds = new Set((ui.knowledge.selectedIds || []).map((id) => String(id || "")));
    panel.querySelectorAll("input[data-knowledge-tag-id]").forEach((input) => {
      input.checked = selectedIds.has(String(input.dataset.knowledgeTagId || ""));
    });
    return true;
  }
  ...
  function renderKnowledgeTagItem(tag) {
    ...
    const editable = tag.is_user_editable !== false;
    return `
      <label class="fp-knowledge-item">
        ...
        <span>
          ...
          <small>${escapeHtml(tag.category || (tag.builtin ? "默认标签" : "自定义"))} · ${editable ? "我的自定义标签" : "系统/历史只读标签"} · ${escapeHtml(stageStatus)}</small>
        ...
        <span class="fp-knowledge-item-actions">
          ...
          ` : `<span class="fp-tag lock">只读</span>`}
        </span>
      </label>
    `;
  }
  ...
  function filteredAssets() {
    ...
    let items = ui.assets.filter((item) => String(item.asset_kind || "framework_planner") === "framework_planner");
    if (query) {
      items = items.filter((item) => `${item.title || ""} ${item.summary || ""}`.toLowerCase().includes(query));
    }
    if (ui.assetStatusFilter !== "all") {
      items = items.filter((item) => assetStatusKey(item) === ui.assetStatusFilter);
    }
    items.sort((a, b) => {
  ...
  function assetCompleted(item) {
    if (item && typeof item === "object") {
      if (Object.prototype.hasOwnProperty.call(item, "asset_completed")) {
        return item.asset_completed === true || item.asset_completed === "true";
      }
      if (String(item.asset_status || "").trim() === "completed") return true;
      return String(item.status || "").trim() === "completed";
    }
    return String(item || "").trim() === "completed";
  }

  function assetStatusKey(item) {
    return assetCompleted(item) ? "completed" : "in_progress";
  }

  function assetStatusLabel(item) {
    if (item && typeof item === "object") {
      const label = String(item.asset_status_label || "").trim();
      if (label) return label;
    }
    return assetCompleted(item) ? "已完成" : "生成中";
  }

  function assetStatusClass(item) {
    return assetCompleted(item) ? "ok" : "blue";
  }
      ...
      <div class="fp-running-card" role="status" aria-live="polite">
        <div class="fp-running-main">
          ...
          <div>
            ...
            <p>已运行 <span data-running-elapsed>${escapeHtml(processingElapsedLabel(stageKey))}</span>。</p>
          </div>
        </div>
  ...
  function renderBasicView() {
    ...
    return `
      <section class="fp-card fp-section">
        ...
        ${locked ? `<div class="fp-inline-warning">基础配置已通过完整校验并应用，后续阶段会读取当前 01 输出。</div>` : ""}
        <div class="fp-grid two">
          <div class="fp-field">
        ...
        <div class="fp-grid two" style="margin-top:14px">
          <div class="fp-field">
            ${renderBasicFieldLabel("目标形式", "target_format")}
          ...
          <div class="fp-field">
            ${renderBasicFieldLabel("总集数", "episodes_per_season")}
            <input type="number" min="1" data-config-key="episodes_per_season" value="${escapeHtml(state.basic_config.episodes_per_season)}" ${runningStageKey() || ui.assetImporting ? "disabled" : ""} />
          </div>
        </div>
        <div class="fp-field" style="margin-top:14px">
          ${renderBasicFieldLabel("每集字数", "episode_word_count")}
          <input type="number" min="1" data-config-key="episode_word_count" value="${escapeHtml(state.basic_config.episode_word_count)}" ${locked ? "disabled" : ""} />
        </div>
        <div class="fp-grid two" style="margin-top:14px">
          <div class="fp-field">
            ${renderBasicFieldLabel("改编方向", "adaptation_direction")}
          ...
          <div class="fp-field">
            ${renderBasicFieldLabel("用户提示词", "user_requirements")}
        ...
        <div class="fp-field" style="margin-top:14px">
          ${renderBasicFieldLabel("原文材料", "source_text")}
        ...
        <div class="fp-field" style="margin-top:14px">
          ${renderBasicFieldLabel("限制条件", "user_constraints")}
  ...
  function renderBasicFieldLabel(label, key) {
    const required = BASIC_REQUIRED_FIELD_KEYS.has(key);
    return `<label>${escapeHtml(label)}${required ? `<span class="fp-required-star" aria-label="必填">*</span>` : ""}</label>`;
  }
  ...
  function renderStagePreferenceField(stageKey, disabled) {
    const preferences = state.prompt_preferences || initialState.prompt_preferences;
    const value = (preferences.manual_stage_prompts || {})[stageKey] || "";
      ...
      <section class="fp-card fp-section">
        <div class="fp-card-title-row">
          <div>
            ...
            <p class="fp-card-sub">04 阶段子视图：三幕十五节拍轴。</p>
  ...
  function renderAssetImportOverlay() {
    return `
      <div class="fp-asset-import-overlay" role="status" aria-live="assertive" aria-busy="true">
        <div class="fp-asset-import-card">
          <div class="fp-asset-import-icon" aria-hidden="true">
            <span class="fp-asset-import-spinner"></span>
          </div>
          <div class="fp-asset-import-copy">
            <span class="fp-asset-import-kicker">正在恢复工作区</span>
            <strong>导入资产中，请稍候</strong>
            <p>正在从后端读取并恢复生成进度，完成后页面会自动打开。</p>
          </div>
          <div class="fp-asset-import-progress" aria-hidden="true"><span></span></div>
          <div class="fp-asset-import-note">
      ...
      <section class="fp-card fp-section">
        <div class="fp-card-title-row">
          <div>
            ...
            <p class="fp-card-sub">04 阶段子视图：卡点说明。</p>
          </div>
          ${stageStatusTag("beat")}
        </div>
...
function renderPackageBlocks() {
  ...
  return `
    <div class="fp-package-wide">
      <div class="fp-panel-card fp-package-card">
        <div class="fp-package-card-head">
          ...
          <button type="button" class="fp-btn small" data-action="toggle-package-details">
            ${ui.packageDetailsExpanded ? "收起完整结构" : "展开完整结构"}
          </button>
        </div>
        ${ui.packageDetailsExpanded
          ? renderDataBlock(state.framework_plan_package, { dataKey: "framework_plan_package", stageKey: "package", editable: false })
          : `
            <div class="fp-output-overview fp-package-lazy-summary">
              <h3>${escapeHtml(extractOutputTitle(state.framework_plan_package, { dataKey: "framework_plan_package" }))}</h3>
              <p>${escapeHtml(extractOutputSummary(state.framework_plan_package))}</p>
              <small>完整策划包已经载入。为加快旧资产打开速度，详细字段改为按需渲染。</small>
            </div>
          `}
      </div>
    </div>
    ...
    <div class="fp-stage-note">
      ...
      <div class="fp-asset-meta">
        ...
        <span>状态：${escapeHtml(assetStatusLabel(state.asset_state || {}))}</span>
      </div>
    </div>
  `;
}
  ...
  function frameworkDownloadBaseName() {
    return String(state.basic_config.source_title || state.basic_config.project_title || "structured_framework")
      .replace(/[\\/:*?"<>|]+/g, "_")
  ...
  function buildReadableFrameworkText() {
    ...
    const title = firstValue(basic.source_title, basic.project_title, packageValue.title, "\u672a\u547d\u540d\u6846\u67b6");
  ...
  function buildStructuredFrameworkExport() {
    ...
    return stripRawResponseKeys({
      ...
      project_title: basic.source_title || basic.project_title || "",
      title: basic.source_title || basic.project_title || "",
      episodes_per_season: basic.episodes_per_season,
      episode_word_count: basic.episode_word_count,
      chars_per_episode: basic.episode_word_count,
  ...
  function extractSourceBriefPayload(response) {
    ...
    ].find((item) => typeof item === "string" && item.trim()) || buildSourceBriefReadableSummary(sourceBrief);
    return { source_brief: sourceBrief, display_text: displayText };
  }

  function buildSourceBriefReadableSummary(sourceBrief) {
    if (!sourceBrief || typeof sourceBrief !== "object" || Array.isArray(sourceBrief)) return "";
    const pieces = [];
    const title = cleanSourceBriefPlaceholder(sourceBrief.title || sourceBrief.source_title || sourceBrief.project_title || "");
    const genre = cleanSourceBriefPlaceholder(sourceBrief.genre || "");
    const tone = cleanSourceBriefPlaceholder(sourceBrief.tone || "");
    const logline = cleanSourceBriefPlaceholder(sourceBrief.core_logline || sourceBrief.core_premise || "");
    const protagonist = cleanSourceBriefPlaceholder(sourceBrief.protagonist || "");
    const conflict = cleanSourceBriefPlaceholder(sourceBrief.core_conflict || sourceBrief.main_opposition || "");
    const material = cleanSourceBriefPlaceholder(sourceBrief.available_material_summary || "");
    if (title) pieces.push(`作品《${title}》已完成基础信息提取。`);
    if (genre || tone) pieces.push(`题材/基调：${[genre, tone].filter(Boolean).join("，")}。`);
    if (logline) pieces.push(`故事核心：${logline}`);
    if (protagonist) pieces.push(`主角信息：${summarizeReadableValue(protagonist)}`);
    if (conflict) pieces.push(`核心冲突：${summarizeReadableValue(conflict)}`);
    if (material) pieces.push(`材料情况：${summarizeReadableValue(material)}`);
    return pieces.join("\n");
  }
  ...
  function renderSourceBriefTree(sourceBrief, displayText) {
    const readableText = cleanSourceBriefPlaceholder(displayText || buildSourceBriefReadableSummary(sourceBrief));
  ...
  function renderReadableFieldCard(label, value, options = {}) {
    ...
    const wideClass = options.wide ? " fp-readable-item-wide" : "";
    return `
      <div class="fp-readable-item${wideClass}">
        <strong>${escapeHtml(label)}</strong>
  ...
  function shouldUseFullWidthReadableCard(stageKey, key) {
    if (stageKey !== "package") return false;
    return [
      "downstream_ready",
  ...
  function renderWhitelistFields(stageKey, data) {
    ...
    pairs.forEach(([key, label]) => {
      ...
      if (isRenderableValue(value)) {
        cards.push({ key, label, value, wide: shouldUseFullWidthReadableCard(stageKey, key) });
      }
    });
    if (!cards.length && data && typeof data === "object" && !Array.isArray(data)) {
      Object.keys(data).filter((key) => !isHiddenTechnicalKey(key) && isRenderableValue(data[key])).slice(0, 6)
        .forEach((key) => cards.push({ key, label: fieldLabel(key), value: data[key], wide: shouldUseFullWidthReadableCard(stageKey, key) }));
    }
    return `<div class="fp-readable-grid">${cards.map((item) => renderReadableFieldCard(item.label, item.value, { wide: item.wide })).join("") || `<div class="fp-empty small">暂无可读字段。</div>`}</div>`;
  }
  ...
  function renderCharacterCards(data) {
    ...
    const unique = [];
    const seen = new Set();
    const identityValues = (item, fallback) => [
      item.id,
      item.character_id,
      item.characterId,
      item.name,
      ...
      .filter(Boolean);
    const characterDisplayName = (item) => item.name
      || item.legal_name
      || item.legalName
      || item.character_name
      || item.characterName
    ...
    characters.forEach((item, index) => {
      ...
      const ids = identityValues(item, index);
    ...
    const fields = [
      ["name", "legal_name", "legalName", "character_name", "characterName", "display_name", "displayName", "title"],
      ["role", "identity"],
      ["external_goal", "goal"],
    ...
    return `<div class="fp-character-grid">${unique.map((item) => `
      <article class="fp-character-card">
        <h3>${escapeHtml(characterDisplayName(item))}</h3>
  ...
  async function goNextStage(nextViewId) {
    ...
    if (nextView.stageKey === "beat") {
      const missing = missingBasicRequiredFields("beat");
      if (missing.length) {
        state.current_view = "basic";
        saveState();
        showBasicRequiredPrompt(missing, "beat");
        return;
      }
      if (!hasSavedFrameworkProjectId() && hasBasicStageOutputForAsset()) {
        try {
          ...
          showToast(`进入 04 前保存框架资产失败：${(error && error.message) || "未知错误"}`);
          return;
        }
      }
    }
  ...
  function firstPositiveInteger() {
    for (let index = 0; index < arguments.length; index += 1) {
      const value = positiveIntegerFromValue(arguments[index]);
      if (value > 0) return value;
    }
    return 0;
  }

  function episodeConfigCandidatesFrom(source) {
    if (!source || typeof source !== "object" || Array.isArray(source)) return [];
    const nested = [
      source.basic_config,
      source.basicConfig,
      source.locked_basic_config,
      source.lockedBasicConfig,
      source.season_plan,
      source.seasonPlan,
      source.episode_count_guard,
      source.episodeCountGuard,
      source.source_brief,
      source.sourceBrief,
    ].filter((item) => item && typeof item === "object" && !Array.isArray(item));
    return [
      ...
      source.episodes_per_season,
      source.episodesPerSeason,
      source.episode_count,
      source.episodeCount,
      ...nested.flatMap(episodeConfigCandidatesFrom),
    ];
  }

  function episodeWordCountCandidatesFrom(source) {
    if (!source || typeof source !== "object" || Array.isArray(source)) return [];
    const nested = [
      source.basic_config,
      source.basicConfig,
    ...
    return [
      source.episode_word_count,
      source.episodeWordCount,
      source.chars_per_episode,
      source.charsPerEpisode,
      ...nested.flatMap(episodeWordCountCandidatesFrom),
    ];
  }

  function syncEpisodeConfigFromKnownSources() {
    if (!state || !state.basic_config) return;
    const basic = state.basic_config;
    const packageBasic = state.framework_plan_package && typeof state.framework_plan_package === "object"
      ? state.framework_plan_package.basic_config || state.framework_plan_package.basicConfig || {}
      : {};
    const episodeCount = firstPositiveInteger(
      basic.episodes_per_season,
      basic.total_episodes,
      ...episodeConfigCandidatesFrom(state.source_brief),
      ...episodeConfigCandidatesFrom(packageBasic),
      ...episodeConfigCandidatesFrom(state.framework_plan_package),
      ...episodeConfigCandidatesFrom(state.asset_state)
    );
    if (episodeCount > 0) {
      basic.episodes_per_season = episodeCount;
      basic.total_episodes = episodeCount;
    }
    basic.season_count = DEFAULT_SEASON_COUNT;

    const wordCount = firstPositiveInteger(
      basic.episode_word_count,
      basic.chars_per_episode,
      ...episodeWordCountCandidatesFrom(state.source_brief),
      ...episodeWordCountCandidatesFrom(packageBasic),
      ...episodeWordCountCandidatesFrom(state.framework_plan_package),
      ...episodeWordCountCandidatesFrom(state.asset_state)
    ) || DEFAULT_EPISODE_WORD_COUNT;
    basic.episode_word_count = wordCount;
    basic.chars_per_episode = wordCount;
  }
  ...
  function currentEpisodeConfig() {
    ...
    syncEpisodeConfigFromKnownSources();
    const basic = state.basic_config || {};
    const seasonCount = DEFAULT_SEASON_COUNT;
    basic.season_count = seasonCount;
    const episodesPerSeason = requiredPositiveInteger(basic.episodes_per_season, "总集数");
  ...
  function frameworkToScriptPayload() {
    ...
    const episodeWordCount = positiveNumber(basic.episode_word_count, DEFAULT_EPISODE_WORD_COUNT);
    const title = String(basic.source_title || basic.project_title || "未命名框架剧本").trim();
    const normalizedBasic = Object.assign({}, basic, {
      ...
      chars_per_episode: episodeWordCount,
    });
    const payload = {
      title,
      project_title: title,
  ...
  function frameworkAssetSavePayload() {
    ...
    state.basic_config.chars_per_episode = episodeWordCount;
    ...
    const payload = Object.assign({
      ...
      source_title: title,
      title,
      season_count: episodeConfig.season_count,
      episodes_per_season: episodeConfig.episodes_per_season,
      total_episodes: episodeConfig.total_episodes,
      episode_word_count: episodeWordCount,
      chars_per_episode: episodeWordCount,
      basic_config: clone(state.basic_config),
      source_brief: clone(state.source_brief),
    ...
    payload.prompt_preferences = normalizePromptPreferences(state.prompt_preferences || {});
  ...
  function syncBasicConfigFromDom() {
    ...
    let sawSourceTitleInput = false;
    app.querySelectorAll("[data-config-key]").forEach((field) => {
      const key = field.dataset.configKey;
      if (!key || field.disabled) return;
      if (key === "source_title") sawSourceTitleInput = true;
      state.basic_config[key] = field.type === "number" ? Number(field.value) : field.value;
    });
    syncBasicTitles({ allowProjectFallback: !sawSourceTitleInput });
    state.basic_config.season_count = DEFAULT_SEASON_COUNT;
    state.basic_config.episode_word_count = positiveNumber(state.basic_config.episode_word_count || state.basic_config.chars_per_episode, DEFAULT_EPISODE_WORD_COUNT);
    state.basic_config.chars_per_episode = state.basic_config.episode_word_count;
  }

  function basicFieldValue(key) {
    if (key === "source_title") return syncBasicTitles({ allowProjectFallback: false });
    if (key === "user_requirements") return payloadUserRequirements();
    return (state.basic_config || {})[key];
  }

  function fieldsForValidationScope(scope) {
    const scopeKey = String(scope || "all");
    if (scopeKey === "all") return BASIC_REQUIRED_FIELDS.slice();
    return BASIC_REQUIRED_FIELDS.filter((field) => {
      const scopes = Array.isArray(field.scopes) ? field.scopes : [];
      return scopes.includes(scopeKey);
    });
  }

  function missingBasicRequiredFields(scope) {
    syncBasicConfigFromDom();
    if (scope === "beat" || scope === "script" || scope === "all") {
      syncEpisodeConfigFromKnownSources();
    }
    return fieldsForValidationScope(scope).filter((field) => {
      const value = basicFieldValue(field.key);
      if (field.positiveInteger) {
        return positiveIntegerFromValue(value) <= 0;
      }
      return !String(value === undefined || value === null ? "" : value).trim();
    });
  }

  function showBasicRequiredPrompt(fields, scope) {
    const missing = Array.isArray(fields) ? fields : missingBasicRequiredFields(scope);
    if (!missing.length) return false;
    const scopeKey = String(scope || "all");
    const title = scopeKey === "beat"
      ? "请先填写总集数"
      : scopeKey === "basic"
        ? "请先完整填写第一阶段基础信息"
        : "请先补全带星号的基础信息";
    const message = scopeKey === "beat"
      ? "04 节拍规划需要明确总集数；当前项目默认一季，总集数会直接作为后续集数。"
      : scopeKey === "basic"
        ? "第一阶段不会在信息缺失时调用工作流。请补全下列字段后再生成，确保 02-12 阶段所需的基础信息完整。"
        : "这些字段会影响第一阶段生成，请补全后再继续。";
    ui.validationPrompt = {
      title,
      message,
      fields: missing,
  ...
  function ensureBasicRequiredFields(scope) {
    const missing = missingBasicRequiredFields(scope);
    if (missing.length) {
      showBasicRequiredPrompt(missing, scope);
      return false;
    }
    return true;
  }
  ...
  function collectNewScriptFormFromDom() {
    ...
    next.season_count = DEFAULT_SEASON_COUNT;
    next.episode_word_count = positiveNumber(next.episode_word_count, DEFAULT_EPISODE_WORD_COUNT);
  ...
  function buildStagePayload(stageKey, options) {
    ...
    if (stageKey === "basic") {
      ...
      return {
        ...
        chars_per_episode: state.basic_config.episode_word_count,
        adaptation_direction: state.basic_config.adaptation_direction,
        user_constraints: state.basic_config.user_constraints,
        user_requirements: payloadUserRequirements(),
      };
  ...
  function attachProjectContext(payload) {
    ...
    next.project_title = title;
    next.source_title = title;
    return next;
  }
  ...
  async function loadStageHistory(stageKey, options = {}) {
    ...
    if (!hasSavedFrameworkProjectId()) {
      ui.stageHistory[stageKey] = [];
      ui.stageHistoryLoading[stageKey] = false;
      if (!silent) render({ skipPersist: true });
      return;
    }
  ...
  async function loadHistoryVersion(stageKey, filename) {
    if (!filename) return;
    if (!hasSavedFrameworkProjectId()) {
      showToast("当前框架尚未保存为资产，暂无可恢复的后端历史版本。");
      return;
    }
  ...
  function applyStageResponse(stageNo, response) {
    ...
    if (stageNo === "07") {
      ...
      state.validation_report = safeData.validation_report && typeof safeData.validation_report === "object" && !Array.isArray(safeData.validation_report)
        ...
        : {};
      ui.packageDetailsExpanded = true;
    }
    syncStageFlow(state);
    markDirty();
  }
  ...
  function formatStageError(error, stageNo) {
    ...
    if (/无法连接 工作流 服务/.test(message)) {
      const parts = [`${label} 无法连接 工作流 服务`];
      if (endpoint) parts.push(`当前 endpoint：${endpoint}`);
      parts.push(suggestion || "建议检查 IP、端口、防火墙、工作流 服务状态。");
      return parts.join("\n");
    }
  ...
  async function confirmBasic() {
    if (!ensureBasicRequiredFields("basic")) {
      return;
    }
    try {
...
async function saveFrameworkAsset(options) {
  ...
  try {
        ...
        announceAssetChange("framework-saved", { project_id: projectId });
        await loadAssets({ silent: safeOptions.silent }).catch((error) => {
          if (!safeOptions.silent) showToast(`框架已保存，但列表刷新失败：${error.message || "请稍后手动刷新"}`);
        });
        return asset;
      } catch (error) {
        if (!safeOptions.silent) {
  ...
  async function loadKnowledgePreferences() {
    try {
      ...
      state.prompt_preferences = normalizePromptPreferences(Object.assign({}, state.prompt_preferences || {}, {
        ...
        manual_stage_prompts: preferences.manual_stage_prompts || {},
        tag_stage_prompts: preferences.tag_stage_prompts || {},
      }));
      saveState();
      render();
    } catch (error) {
  ...
  function setKnowledgeSelection(tagId, selected) {
    ...
    if (!id) return;
    const ids = new Set((ui.knowledge.selectedIds || []).map((item) => String(item || "").trim()).filter(Boolean));
    const before = ids.has(id);
    if (selected) ids.add(id);
    else ids.delete(id);
    const after = ids.has(id);
    ui.knowledge.selectedIds = Array.from(ids);
    if (before !== after) saveState();
    if (!refreshKnowledgeSelectionUi()) render();
  }
  ...
  async function applyKnowledgeTags() {
    ...
    try {
      ...
      state.prompt_preferences = normalizePromptPreferences(Object.assign({}, state.prompt_preferences || {}, {
        ...
        tag_stage_prompts: stagePrompts,
        active_template_id: "custom",
      }));
  ...
  function openCurrentStagePreferenceEditor(stageKey) {
    ...
    const tag = selected.find((item) => item.is_user_editable !== false)
      || (ui.knowledge.tags || []).find((item) => item.is_user_editable !== false);
    if (tag && tag.id) {
  ...
  async function loadAssets(options = {}) {
    ...
    try {
      ...
      ui.assetsLastRefreshedAt = new Date().toISOString();
    } catch (error) {
      if (!silent) showToast(error.message || "资产列表加载失败");
      throw error;
    } finally {
      if (!silent) ui.assetsLoading = false;
      if (ui.assetsOpen) render();
    }
  }
  ...
  async function createNewScript() {
    ...
    try {
      form.season_count = DEFAULT_SEASON_COUNT;
      form.episodes_per_season = requiredPositiveInteger(form.episodes_per_season, "总集数");
    ...
    resetTransientUi();
    state = clone(initialState);
    state.basic_config.source_title = asset.title || form.title || "未命名剧本";
    syncBasicTitles();
    ...
    state.basic_config.episode_word_count = DEFAULT_EPISODE_WORD_COUNT;
    state.basic_config.chars_per_episode = DEFAULT_EPISODE_WORD_COUNT;
    ...
    state.asset_state.status = "in_progress";
    state.current_view = "basic";
    ui.assetsOpen = true;
    syncStageFlow(state);
    saveState();
    ...
    debugFrontendEvent("new_script_created", {
      ...
      project_title: state.basic_config.source_title,
    ...
    announceAssetChange("framework-created", { project_id: state.asset_state.asset_id });
    await loadAssets();
  ...
  async function deleteAsset(projectId) {
    if (!projectId) return;
    if (!window.confirm("确认删除这个框架资产吗？关联的 08-12 剧本资产也会一并删除。")) return;
    await requestJson(`/api/projects/${projectId}`, { method: "DELETE" });
    announceAssetChange("framework-deleted", { project_id: projectId });
    await loadAssets();
    showToast("资产已删除");
  }
  ...
  function restoreFrameworkPlannerState(project, options = {}) {
    ...
    state.adaptation_guide = normalizeGuideFields(state.adaptation_guide);
    if (!state.basic_config || typeof state.basic_config !== "object") {
      state.basic_config = clone(initialState.basic_config);
    }
    syncBasicTitles();
    state.basic_config.season_count = DEFAULT_SEASON_COUNT;
    ...
    if (!isEmptyValue(state.framework_plan_package)) {
      ...
      ui.packageDetailsExpanded = false;
    } else {
  ...
  async function openAsset(projectId) {
    ...
    ui.toast = "";
    render({ skipPersist: true });
    const importStartedAt = window.performance && typeof window.performance.now === "function"
    ...
    try {
      ...
      if (String(project.asset_kind || "") === "framework_planner") {
        restoreFrameworkPlannerState(project, { skipPersist: true });
        ui.stageHistory = {};
        ui.stageHistoryLoading = {};
        clearDirty();
        imported = true;
      ...
      state.basic_config.episode_word_count = positiveNumber(input.episode_word_count || input.chars_per_episode || state.basic_config.episode_word_count || state.basic_config.chars_per_episode, DEFAULT_EPISODE_WORD_COUNT);
      state.basic_config.chars_per_episode = state.basic_config.episode_word_count;
      ...
      state.asset_state.status = project.status || "in_progress";
      state.asset_state.asset_completed = project.asset_completed === true || project.asset_completed === "true";
      state.asset_state.asset_status = project.asset_status || assetStatusKey(project);
      state.asset_state.asset_status_label = project.asset_status_label || assetStatusLabel(project);
      state.current_view = "basic";
      ui.stageHistory = {};
      ui.stageHistoryLoading = {};
      syncStageFlow(state);
      ...
      if (imported) {
        saveState();
        render({ skipSync: true, skipPersist: true });
        showToast("资产导入成功", { render: false });
        const importFinishedAt = window.performance && typeof window.performance.now === "function"
          ? window.performance.now()
          : Date.now();
        debugStageSummary("asset_import_performance", {
          project_id: currentProjectId(),
          current_view: state.current_view,
          elapsed_ms: Math.round(importFinishedAt - importStartedAt),
          package_details_deferred: !ui.packageDetailsExpanded && !isEmptyValue(state.framework_plan_package),
        });
      } else {
        render({ skipPersist: true });
      }
  ...
  async function controlAssetTask(taskId, action) {
    ...
    announceAssetChange("framework-task-updated", { task_id: taskId, action });
    await loadAssets();
  ...
  app.addEventListener("input", (event) => {
    ...
    if (target.matches("[data-config-key]")) {
      ...
      if (key === "episodes_per_season") {
        const episodeCount = positiveIntegerFromValue(target.value);
        state.basic_config.total_episodes = episodeCount > 0 ? episodeCount : "";
      }
      if (key === "source_title") syncBasicTitles({ allowProjectFallback: false });
      markDirty();
    ...
    if (target.matches("[data-stage-preference-key]")) {
      ...
      state.prompt_preferences.manual_stage_prompts = normalizeStagePrompts(state.prompt_preferences.manual_stage_prompts || {});
      state.prompt_preferences.manual_stage_prompts[stageKey] = target.value;
      state.prompt_preferences.stage_prompts = mergeStagePromptsNonEmpty(
        state.prompt_preferences.manual_stage_prompts,
        state.prompt_preferences.tag_stage_prompts || {}
      );
      markDirty();
  ...
  app.addEventListener("click", async (event) => {
    ...
    if (action === "close-validation-prompt") {
      if (actionElement.matches(".fp-modal-mask") && event.target !== actionElement) return;
      ui.validationPrompt = null;
      render();
      return;
    }
    if (action === "go-view") {
    ...
    if (action === "toggle-package-details") {
      ui.packageDetailsExpanded = !ui.packageDetailsExpanded;
      render({ skipPersist: true });
      return;
    }
    ...
    if (action === "run-stage-generate") {
      ...
      try {
        if (stageKey === "basic" && !ensureBasicRequiredFields("basic")) {
          return;
        }
        if (stageKey === "beat" && !ensureBasicRequiredFields("beat")) {
          return;
        }
  ...
  const requestedProjectId = Number(new URLSearchParams(window.location.search || "").get("project_id") || 0);
  loadAssets()
    .then(() => {
      if (requestedProjectId > 0) return openAsset(requestedProjectId);
      return null;
    })
    .catch(() => {});
