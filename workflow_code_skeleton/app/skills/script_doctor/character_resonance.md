...
- **足够简明**：以上所有可识别、可理解的内容，是否能让一个第一次接触文本的普通读者，在人物首次登场后很短的篇幅内就抓住重点，而不需要反复回读或等到很后面才拼凑出人物是谁。共鸣的前提是"看得懂"，看不懂就谈不上共不共鸣。
...
- **情感浓缩不等于夸张空洞**：不少爆款内容习惯用争吵、痛哭、和解等强烈、浓缩化的情绪表达让普遍情感更快抵达读者，这种浓缩本身没有问题；但每一次浓缩表达都必须有具体触发和实际代价支撑，否则会滑向单纯卖惨、悬浮喊话或情绪堆砌，反而损害共鸣。
...
成熟创作者在处理需要快速建立记忆点的人物时，常用"先定性、再细化、后印证"的顺序，这一方法可以显著提升共鸣效率，审查与提修改建议时可参照：

1. **一句话标签**：能否用一句话概括人物的核心特质与处境（例如"外表强势内心自卑的单亲妈妈""嘴硬心软的落魄少爷"）？这个标签不是给人物贴刻板印象，而是给读者一个最快的识别入口。
2. **内在品格 + 外在细节细化**：标签之后，是否有具体的内在品格（价值观、判断方式）与外在细节（习惯动作、随身物件、说话方式）反复支撑这个标签？
3. **先亮标签，再展细节**：文本呈现顺序上，是否先让读者快速抓住人物是谁，再逐步展开细节，而不是把标签和细节混杂在一起、让读者自己拼凑？
4. **标签之外的突破**：在标签立住之后，是否给人物暴露了标签之外的柔软面、矛盾面或成长空间，避免人物停留在扁平标签上？
5. 配角可以按"扁平/圆形""成长/非成长"分级处理：单纯推动剧情、露面很少的配角可用扁平但清晰的标签；反复出现或对主线有实质影响的配角，即使不走成长弧光，也至少要把其核心特质（如"野心勃勃"）写透，让读者留下印象，而不是停留在功能工具人层面。

## 三句话读懂测试（新增）

对每个主要人物的首次重要登场执行以下测试：

- 尝试只用不超过三句话，回答"这个人是谁、当下处境如何、此刻想要什么"；如果必须依赖后续很多篇幅的补充说明才能回答清楚，说明这次登场的信息密度和呈现顺序需要调整。
- 检查这三句话是否能被一个从未读过原文的人轻松复述；能复述说明画像清晰，复述吃力或容易复述错说明存在标签模糊、信息堆叠或呈现顺序问题。
- 将测试结果和判断依据写入 `character_entry_hooks` 与新增字段 `clarity_score` 中。
...
3. 分别评估理解、喜欢、钦佩、心疼、好奇、代入六条吸引路径，并额外执行"三句话读懂测试"给出 `clarity_score`。
4. 用"标签先行，行为印证"的框架检查每个主要人物与关键配角，判断标签是否清晰、细节是否支撑、是否有标签之外的突破。
5. 找出画像同质化、标签化、过度完美、只卖惨、缺乏主动选择、代价虚假、关系单向等断裂点。
6. 找出已有的高潜力共鸣瞬间，优先放大有效内容，不为了显得深刻而新增悲惨身世。
7. 为高优先级问题给出可执行的场景级修改，并提供 50-150 字局部样例或具体动作方案。
8. 单独审查主要人物第一次重要登场，给出其"人物承诺、情感问题、信息缺口和最迟兑现位置"。
  ...
  "character_entry_hooks": [
    {
      ...
      "three_sentence_test_result": "pass|partial|fail",
      "issue": "string",
