...
from ..services.workflow_errors import (
    WorkflowPayloadTooLargeError,
    WorkflowStageFormatError,
...
SCRIPT_STAGE_PAYLOAD_HARD_LIMIT = max(
    240000,
    int(getattr(settings, "workflow_script_payload_hard_limit", 240000)),
)
SCRIPT_STAGE_PAYLOAD_SOFT_LIMIT = min(
    SCRIPT_STAGE_PAYLOAD_HARD_LIMIT - 20000,
    max(
        200000,
        int(getattr(settings, "workflow_script_payload_soft_limit", 200000)),
    ),
)
SCRIPT_STAGE_PREVIOUS_SCRIPT_BATCHES = 2
SCRIPT_STAGE_PREVIOUS_BATCH_SUMMARY_MAX_CHARS = max(
    1200,
    int(getattr(settings, "workflow_script_previous_batch_summary_max_chars", 4500)),
)
SCRIPT_STAGE_MEMORY_MAX_CHARS = max(
    2000,
    int(getattr(settings, "workflow_script_memory_max_chars", 8000)),
)
...
def run_workflow_orchestrator(
    ...
    *,
    ...
    client: WorkflowRunner | None = None,
    resume_snapshot: dict[str, Any] | None = None,
) -> WorkflowState:
    """主控整个 工作流 编排流程，并在关键节点兜底恢复/回退/重试。"""
    ...
    variables = _initial_workflow_variables(payload)
...
def _initial_workflow_variables(payload: WorkflowInput) -> dict[str, Any]:
    return {
        ...
        # framework 真正发给 工作流 的输入仍严格受 contract.input_names 约束，
        ...
        "selected_preference_tags": payload.selected_preference_tags,
...
def _run_batched_generation(
    ...
    if batch_mode in {"workflow_full", "full", "legacy_full"}:
        logger.warning(
            "检测到 WORKFLOW_BATCH_MODE=%s，但当前项目已强制使用本地三段式批处理模式：all_hooks -> all_dialogues -> all_script。",
            batch_mode,
        )
        batch_mode = "local"
    if batch_mode != "local":
        raise ValueError(
            "WORKFLOW_BATCH_MODE 只能是 auto、local 或 workflow_full，"
            f"当前为：{settings.workflow_batch_mode}"
        )
...
def _normalize_framework_to_script_asset_variables(variables: dict[str, Any]) -> None:
    ...
    工作流 variableUpdate often stores a full answerText JSON string into an
...
def _run_framework_to_script_workflow(
    ...
    for index, batch in enumerate(batches, start=1):
        ...
        memory_output = _run_workflow_stage(
            state,
            runner,
            STAGE_FRAMEWORK_CAUSAL_CONFLICT_MEMORY,
            {
        ...
        script_memory_output = _run_workflow_stage(
            state,
            runner,
            STAGE_FRAMEWORK_SCRIPT_MEMORY,
            {
...
def _stage_name_for_runner(runner: WorkflowRunner, preferred: str, legacy: str) -> str:
...
def _stage_review_revise_loop_limit() -> int:
    configured = int(
        getattr(settings, "workflow_stage_review_revise_max_loops", BATCH_REVIEW_MAX_LOOPS)
    )
...
def _effective_batch_mode() -> str:
    mode = settings.workflow_batch_mode
    if mode == "auto":
        return "local"
    return mode
...
def _run_full_workflow_generation(
    state: WorkflowState,
    runner: WorkflowRunner,
    payload: WorkflowInput,
    variables: dict[str, Any],
    ...
    hook_output = _run_workflow_stage(
        state,
        runner,
        STAGE_HOOKS,
        variables,
    ...
    dialogue_output = _run_workflow_stage(
        state,
        runner,
        STAGE_DIALOGUES,
        variables,
    ...
    script_output = _run_workflow_stage(
        state,
        runner,
        STAGE_SCRIPT,
        script_variables,
...
def _stage_format_retry_limit() -> int:
    configured = int(getattr(settings, "workflow_stage_format_retry_limit", 3))
    return max(1, configured)


def _runner_stage_debug_info(runner: WorkflowRunner, stage_name: str) -> dict[str, Any]:
    ...
    *,
    ...
    payload = {
        **artifact,
        ...
        "workflow_client_answer_text_preview": str(
            runner_debug.get("answer_text_preview") or ""
        ),
        "workflow_client_response_preview": str(
            runner_debug.get("response_preview") or ""
        ),
        "workflow_client_output_keys": list(runner_debug.get("output_keys") or []),
        "workflow_client_raw_response": runner_debug.get("raw_response"),
        "workflow_client_last_stage_debug_info": _compact_nested_strings(
            runner_debug,
            max_chars=600,
        ),
...
def _run_workflow_stage_once(
    ...
    *,
    ...
    _log_workflow_stage_start(state, log_label, batch_label, 1)
    ...
    _log_workflow_stage_done(state, log_label, batch_label, output)
...
def run_stage_with_contract_guard(
    ...
    *,
    ...
    while format_attempt < retries:
        ...
        try:
            raw_output = _run_workflow_stage_once(
                state,
                runner,
                stage_name,
                variables,
...
def _run_workflow_stage(
    ...
    *,
    ...
    """统一封装单个 工作流 阶段调用，负责进度上报、契约校验和网络重试。"""
    contract = contract_for(stage_name)
    runtime_stage_key = stage_name or stage_key
    del max_retries
    ...
    while True:
        ...
        sync_runtime_state(state)
        try:
            output = _run_workflow_stage_once(
                state,
                runner,
                stage_name,
                variables,
...
def _ensure_scene_outputs(
    ...
    try:
        ...
        logger.warning(
            "stage_name=scenes failure_type=scene_stage_restart_exhausted source=workflow_stage "
            "missing_fields=[] attempted_json_repair=%s local_restart_attempt=%s "
            "cleared_stage_cache=%s preview=%s",
            True,
            max(0, int(getattr(settings, "workflow_stage_local_restart_retries", 1))),
            True,
...
def _try_generate_appearance_unstructured_output(
    ...
    try:
        ...
        logger.warning(
            ...
            "source=workflow_stage blocking_issues=[] cleared_stage_cache=%s preview=%s",
            False,
            _truncate_log_text(str(exc), max_chars=320),
        )
        state.set_output(
...
def _ensure_stage_output_mapping(stage_name: str, output: Any) -> dict[str, Any]:
    if isinstance(output, dict):
        return output
    if output is None:
        raise ValueError(f"工作流 阶段 {stage_name} 返回了空结果（None）。")
    ...
    raise ValueError(
        f"工作流 阶段 {stage_name} 返回了非对象结果（{type(output).__name__}）{suffix}"
    )


def _log_workflow_stage_start(
    state: WorkflowState,
    stage_label: str,
    batch_label: str | None,
    attempt: int,
    ...
    if runtime and hasattr(runtime, "workflow_stage_started"):
        runtime.workflow_stage_started(stage_label, batch_label=batch_label, attempt=attempt)
...
def _log_workflow_stage_done(
    ...
    runtime = state.runtime
    if runtime and hasattr(runtime, "workflow_stage_finished"):
        runtime.workflow_stage_finished(stage_label, batch_label=batch_label, output=output)
...
def _is_non_retryable(exc: Exception) -> bool:
    if isinstance(exc, WorkflowPayloadTooLargeError):
        return True
    text = str(exc)
    return (
        "缺少 工作流 API Key" in text
        or "缺少三幕十五节拍框架转剧本专用 工作流 API Key" in text
        or "401" in text
        or "403" in text
    )
...
def _transient_retry_delay(attempt: int) -> float:
    base = max(1.0, float(getattr(settings, "workflow_http_retry_delay", 1.5)))
...
def _strict_consistency_flag(value: Any) -> bool | None:
    if value in (None, ""):
        return None
    try:
        return coerce_strict_workflow_boolean(value)
    except ValueError:
        return None
...
def _ensure_pre_strategy_outputs(
    ...
    pre_strategy_output = _run_workflow_stage(
        state,
        runner,
        STAGE_APPEARANCE_PRE_STRATEGY,
        variables,
...
def _ensure_framework_outputs(
    ...
    *,
    ...
    for attempt in range(1, total_attempts + 1):
        _reset_workflow_to_initial_input_state(state, payload, variables)
        try:
            framework_output = _run_workflow_stage(
                state,
                runner,
                STAGE_FRAMEWORK,
                variables,
...
def _run_consistency_stage_with_self_check(
    ...
    for check_attempt in range(1, CONSISTENCY_SELF_CHECK_ATTEMPTS + 1):
        ...
        try:
            consistency = _run_workflow_stage(
                state,
                runner,
                STAGE_CONSISTENCY,
                variables,
...
def _reset_workflow_to_initial_input_state(
    ...
    variables.clear()
    variables.update(_initial_workflow_variables(payload))
...
def _apply_appearance_outputs_to_variables(variables: dict[str, Any]) -> bool:
    ...
    normalized_plan = _normalize_episode_plan_object(variables.get(NORMALIZED_EPISODE_PLAN))
    # appearanceMapping 是 工作流 给出的“原始服装/别名母本”；
