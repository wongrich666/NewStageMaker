...
def story_bible_contract_instruction(request_payload: dict) -> str:
    bible = str(request_payload.get("continuation_bible") or "").strip()
    raw_enabled = request_payload.get("story_bible_enabled")
    enabled = (
        bool(bible)
    ...
    if not enabled or not bible:
        return ""
    mode = str(request_payload.get("mode") or "原创").strip()
    if mode == "续写":
...
def continuation_contract_instruction(request_payload: dict) -> str:
    story_bible_contract = story_bible_contract_instruction(request_payload)
...
def normalize_scene_card_handoffs(
    text: str,
    *,
    cross_episode: bool = True,
) -> tuple[str, list[str]]:
    """Make every planned destination/action agree with the scene that follows."""
...
def normalize_episode_card_handoffs(text: str) -> tuple[str, list[str]]:
    ...
    output, scene_warnings = normalize_scene_card_handoffs(output)
...
def generate_stage_result(stage: str, request_payload: dict, *, modules: str) -> str:
    ...
    while True:
        ...
        if stage == "episode_continuity":
            if ip_anthology_enabled(request_payload):
                result, handoff_warnings = normalize_scene_card_handoffs(
