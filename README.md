...
在框架策划工作台的“智慧库 / 阶段偏好”里，每个标签旁边有 `✍️` 按钮。点击后可以编辑该标签下的阶段提示词，至少包括 01 原文提取、02 世界观、03 人设、04 节拍规划、05 人物故事线、06 改编指引、07 框架校验，同时保留 08 场景字典、09 确定角色外观、10 分集细化、11 开头冲突钩子、12 正文写作的输入结构。
...
- 06：写作方向、原文保留内容、本次重点改变、风格要求、风险提醒、给后续写作的硬要求。
...
- `下载结构化框架`：导出 `.json`，用于导入框架到剧本工作台。内容包含 `frameworkPlanPackage / framework_plan_package`、`stageOutputs`、标题、集数、分钟数、目标形式、写作方向，以及 `export_version / exported_at` 等元数据；调试 raw、token、nodeId 等字段会被清理。
...
06 “整体改编指引”支持字段化编辑：写作方向、原文保留内容、本次重点改变、风格要求、风险提醒、后续写作硬要求。修改后必须点击“应用修改”，否则 07 生成和保存会被拦截。
